...
create or replace function qvm_new_apps.business_minutes(p_start timestamptz, p_end timestamptz)
returns numeric language plpgsql immutable as $$
declare
  v_tz  text := 'Asia/Riyadh';
  v_day_start int := 8*60;
  v_day_end   int := 18*60;
...
begin
  ...
  v_cursor := v_start_local::date;
  while v_cursor <= v_end_local::date loop
    v_dow := extract(dow from v_cursor);
    if v_dow between 0 and 4 then
      v_win_start := v_cursor + make_interval(mins => v_day_start);
      v_win_end   := v_cursor + make_interval(mins => v_day_end);
      v_seg_start := greatest(v_start_local, v_win_start);
      v_seg_end   := least(v_end_local, v_win_end);
...
create or replace function qvm_new_apps.brand_type_bucket(p_brand_class int)
returns text language sql immutable as $$
  select case
    when p_brand_class in (107,108) then 'oem'
    when p_brand_class in (109,110) then 'aftermarket'
    else 'any'
  end;
$$;
...
declare
  ...
  v_raw_pct numeric; v_adj_pct numeric; v_breakdown jsonb;
begin
  ...
  select jsonb_build_object(
    ...
    'by_employee', coalesce((
       select jsonb_agg(jsonb_build_object(
         'user_id', s2.created_by, 'user_name', ud.user_name,
         'avg_business_minutes', round(avg(s2.bmin),1), 'items', count(*))
         order by avg(s2.bmin))
       from spans s2 left join qvm_new_apps.user_data ud on ud.user_id = s2.created_by
       group by s2.created_by, ud.user_name), '[]'::jsonb)
...
grant execute on function qvm_new_apps.get_purchasing_report(timestamptz,timestamptz,uuid) to authenticated;
grant execute on function qvm_new_apps.business_minutes(timestamptz,timestamptz) to authenticated;
grant execute on function qvm_new_apps.brand_type_bucket(int) to authenticated;
