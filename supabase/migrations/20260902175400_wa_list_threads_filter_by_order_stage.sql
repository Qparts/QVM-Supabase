...
-- Filing the inbox by where the work is, not by a flag somebody remembered to set.
--
-- open / pending / closed described the conversation: three states a person had to
-- maintain by hand, which in practice meant everything sat in "open" forever and the
-- tabs sorted nothing. What a buyer actually wants to see is the orders — the vendors
-- who still owe a price, and the orders that are being fulfilled — and that is already
-- recorded, on every line of every order, by people doing their normal work.
--
-- A conversation's orders are the same ones the context column lists: the RFQs this
-- vendor is on, plus an order pinned to the thread by hand. A thread belongs to a tab
-- if any line of any of those orders is in that stage, so a vendor with one quote
-- outstanding and one delivery in flight appears under both — which is true.
--
-- The old three values still filter, so nothing that calls this with them breaks.

create index if not exists quotation_vendors_vendor_idx
  on qvm_new_apps.quotation_vendors (vendor_id);
...
as $function$
declare
  v_rows jsonb; v_total bigint; v_q text := nullif(btrim(coalesce(p_search,'')),'');
  v_ids bigint[];
