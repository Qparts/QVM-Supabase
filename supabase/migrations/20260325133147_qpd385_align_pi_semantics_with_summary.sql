-- Synced from QVM/test branch applied migration history (version 20260325133147, name: qpd385_align_pi_semantics_with_summary)
-- QPD-385: Align Purchase Invoices Dashboard semantics with get_overall_order_summary
SET search_path TO qvm_new_apps, public;

-- Update counters to follow order_scope + strict PI missing definition (URL, Number, Zoho all empty)
...
AS $$
  ...
  rn_missing AS (
    -- latest PO per order, and missing any vendor credit note
...
-- Update dashboard RPC to use identical order_scope and PI missing semantics; return both row and order totals
...
BEGIN
  ...
  WITH user_ctx AS (
    ...
    SELECT coalesce(array_agg(list_data_id), ARRAY[]::int[]) AS ids
    FROM qvm_new_apps.list_data
    WHERE lower(list_data) LIKE 'deliver%'
  ), po_missing AS (
    -- Semantics aligned with summary: URL, Number, Zoho all empty or no PO rows
    SELECT DISTINCT os.confirmed_order_id
    FROM order_scope os
    LEFT JOIN qvm_new_apps.purchase_orders po
      ON po.confirmed_order_id = os.confirmed_order_id
    ...
    SELECT 
      ...
      po.zoho_bill_url,
      (SELECT sl.status_changed_by FROM qvm_new_apps.status_logs sl WHERE sl.confirmed_item_id = ci.confirmed_item_id AND sl.item_status = ANY((SELECT ids FROM delivered_ids)) ORDER BY sl.created_at DESC LIMIT 1) AS delivered_by
    ...
    WHERE
      ...
      AND (p_branch_ids IS NULL OR i.customer_id = ANY(COALESCE(p_branch_ids, ARRAY[]::int[])))
      AND (p_supplier_ids IS NULL OR i.vendor_id = ANY(COALESCE(p_supplier_ids, ARRAY[]::int[])))
  )
