-- The internal dashboard's Alt Part Number column lists the alternative items.
--
-- Each line in get_internal_dashboard now carries every alternative item on it — the vendors' and
-- the desk's, labelled by source — beside the legacy free-text alternative number.
              ...
              SELECT jsonb_agg(
                jsonb_build_object(
                  ...
                  'alternatives', (SELECT COALESCE(jsonb_agg(jsonb_build_object(
                                     ...
                                     'unit_price', a.unit_price, 'visible_to_workshop', a.visible_to_workshop,
...
CREATE OR REPLACE FUNCTION public.approval_flow_version()
 RETURNS integer LANGUAGE sql STABLE AS $$ SELECT 27 $$;
