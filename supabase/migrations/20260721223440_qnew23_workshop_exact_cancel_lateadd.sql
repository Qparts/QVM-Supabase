...
begin
  ...
  into v_volume
  from (select q.service_advisor, count(distinct q.quotation_id) AS requests, count(qi.quotation_item_id) AS items
    from qvm_new_apps.quotations q left join qvm_new_apps.quotation_items qi on qi.quotation_id = q.quotation_id
    where q.created_at >= v_from and q.created_at <= v_to and (v_emp is null or q.service_advisor = v_emp)
    group by q.service_advisor) e
  ...
  into v_by_city
  from (select cb.city, count(distinct q.quotation_id) AS c
    from qvm_new_apps.quotations q join qvm_new_apps.quotation_items qi on qi.quotation_id = q.quotation_id
    join qvm_new_apps.client_branches cb on cb.customer_id = qi.customer_id
    where q.created_at >= v_from and q.created_at <= v_to and cb.city is not null and (v_emp is null or q.service_advisor = v_emp)
    group by cb.city) z;
  ...
  spans as (
    ...
    from (select qiid, min(created_at) filter (where item_status = 19) AS t_conf, min(created_at) filter (where item_status = 23) AS t_sign
          from sl group by qiid) t
    ...
    where t.t_conf is not null and t.t_sign is not null and t.t_sign > t.t_conf
      and q.created_at >= v_from and q.created_at <= v_to and (v_emp is null or q.service_advisor = v_emp)
  )
  select jsonb_build_object(
    'avg_response_min', coalesce(round(avg(bmin),1),0), 'sample_count', count(*), 'threshold_hours', v_threshold,
    'over_threshold', count(*) filter (where v_threshold is not null and bmin > v_threshold*60),
    'by_employee', coalesce((
      select jsonb_agg(jsonb_build_object('user_id', x.service_advisor, 'user_name', ud.user_name, 'avg_response_min', x.avg_bmin, 'items', x.items) order by x.avg_bmin desc)
      from (select s2.service_advisor, round(avg(s2.bmin),1) AS avg_bmin, count(*) AS items from spans s2 group by s2.service_advisor) x
  ...
  where qi.cancellation_reason is not null
    and not exists (select 1 from qvm_new_apps.status_logs s where s.quotation_item_id = qi.quotation_item_id and s.item_status not in (15,18,268))
    and q.created_at >= v_from and q.created_at <= v_to and (v_emp is null or q.service_advisor = v_emp);

  select count(*) into v_late
  from qvm_new_apps.confirmed_items ci join qvm_new_apps.quotation_items qi on qi.quotation_item_id = ci.quotation_item_id
  join qvm_new_apps.quotations q on q.quotation_id = qi.quotation_id
  ...
  where exists (select 1 from qvm_new_apps.status_logs s join qvm_new_apps.quotation_items qx on qx.quotation_item_id = s.quotation_item_id
                where qx.quotation_id = qi.quotation_id and s.item_status <> 15 and s.created_at < qi.created_at)
    and q.created_at >= v_from and q.created_at <= v_to and (v_emp is null or q.service_advisor = v_emp);
  ...
  where coalesce(n.is_internal, false) is true and coalesce(n.is_deleted,false) = false
    and n.created_at >= v_from and n.created_at <= v_to and (v_emp is null or n.user_id = v_emp);
  ...
  select coalesce(sum(spend),0)::numeric,
    coalesce(jsonb_agg(jsonb_build_object('week', to_char(wkstart,'YYYY-MM-DD'), 'spend', round(spend::numeric,2)) order by wkstart), '[]'::jsonb)
  into v_spend, v_weekly from wk;

  return jsonb_build_object('status', true, 'message', 'ok', 'data', jsonb_build_object(
    'range', jsonb_build_object('from', v_from, 'to', v_to), 'is_admin', v_admin, 'scoped_to_self', (not v_admin),
    'volume', jsonb_build_object('total_requests', v_total_requests, 'total_items', v_total_items, 'by_employee', v_volume, 'by_city', v_by_city),
    'time', v_time,
    'quality', jsonb_build_object('early_cancellations', v_early, 'late_cancellations', v_late, 'late_additions', v_late_add, 'internal_notes', v_notes),
