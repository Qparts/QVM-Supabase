-- Fix get_client_vendor_location_report: group by VENDOR, not by client region. The user clarified
-- the report should list every vendor and, per vendor, classify each of their fulfilled quotations
-- as Local (the client branch that created the quotation is in the same resolved city as the
-- vendor's own branch) or Other City — not the reverse (client region as the row, vendor as the
-- split). Classification logic (nearest-city-by-coordinates on both sides) is unchanged; only the
-- returned identity column changes from client_branch_name to vendor_id/vendor_name.
...
BEGIN
  ...
  WITH scoped_items AS (
    SELECT qi.quotation_item_id, qi.quotation_id, qi.cost_id, qi.customer_id
  ...
  FROM (
    ...
    JOIN qvm_new_apps.vendors v ON v.vendor_id = qvi.vendor_id
