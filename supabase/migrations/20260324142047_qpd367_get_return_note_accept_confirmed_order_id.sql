-- Synced from QVM/test branch applied migration history (version 20260324142047, name: qpd367_get_return_note_accept_confirmed_order_id)
-- QPD-367: Recreate get_return_note to accept optional p_confirmed_order_id for precise selection
DO $$
BEGIN
  IF EXISTS (
    ...
    WHERE n.nspname = 'public'
      AND p.proname = 'get_return_note'
      AND pg_catalog.pg_get_function_identity_arguments(p.oid) = 'p_order_number text, p_return_id integer'
  ) THEN
    DROP FUNCTION public.get_return_note(p_order_number text, p_return_id integer);
  END IF;
END $$;
...
CREATE OR REPLACE FUNCTION public.get_return_note(
  ...
  p_confirmed_order_id int DEFAULT NULL
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  ELSE
    ...
    IF p_confirmed_order_id IS NOT NULL THEN
      SELECT * INTO v_co FROM confirmed_orders WHERE confirmed_order_id = p_confirmed_order_id LIMIT 1;
      IF NOT FOUND THEN
        RETURN jsonb_build_object('status', false, 'message', 'Specified confirmed order not found', 'data', NULL);
      END IF;
    ELSE
      SELECT * INTO v_co
    ...
    -- First try direct link on returns
    SELECT * INTO v_ret
    FROM returns
...
REVOKE EXECUTE ON FUNCTION public.get_return_note(text, int, int) FROM public, anon;
GRANT EXECUTE ON FUNCTION public.get_return_note(text, int, int) TO authenticated;
;
