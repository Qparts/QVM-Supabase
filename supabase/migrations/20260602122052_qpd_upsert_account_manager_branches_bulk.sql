-- Synced from QVM/test branch applied migration history (version 20260602122052, name: qpd_upsert_account_manager_branches_bulk)
BEGIN;
...
BEGIN
  ...
  FOR rec IN SELECT * FROM jsonb_array_elements(p_rows) LOOP
    ...
    INSERT INTO qvm_new_apps.account_manager_branches(
      customer_id,
      slot_number,
      main_account_manager,
      first_substitute,
      second_substitute,
      fallback_account_manager,
      created_at,
      updated_at
    ) VALUES
      (b_id, 1, m1, s11, s21, fb, now(), now()),
      (b_id, 2, m2, s12, s22, fb, now(), now()),
      (b_id, 3, m3, s13, s23, fb, now(), now())
    ON CONFLICT (customer_id, slot_number) DO UPDATE
    SET
      main_account_manager = EXCLUDED.main_account_manager,
      first_substitute = EXCLUDED.first_substitute,
      second_substitute = EXCLUDED.second_substitute,
      fallback_account_manager = EXCLUDED.fallback_account_manager,
      updated_at = now();
