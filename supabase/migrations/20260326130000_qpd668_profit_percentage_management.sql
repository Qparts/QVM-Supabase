-- QPD-668/672: Profit Percentage Management - schema & RPCs
SET search_path TO qvm_new_apps, public;

-- 1) Schema: Branch-specific profit margins (overrides)
...
DO $$ BEGIN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE tablename = 'profit_margins' AND indexname = 'uq_profit_margins'
  ) THEN
    EXECUTE 'CREATE UNIQUE INDEX uq_profit_margins ON qvm_new_apps.profit_margins(profit_categories_id, cost_range_id)';
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE tablename = 'profit_categories' AND indexname = 'uq_profit_categories'
  ) THEN
    EXECUTE 'CREATE UNIQUE INDEX uq_profit_categories ON qvm_new_apps.profit_categories(brand_class, part_category)';
  END IF;
END $$;
...
DECLARE
  rows jsonb;
BEGIN
  -- Authorization: allow Admin, Finance Manager, Pricing Supervisor, Account Manager to view
  IF NOT EXISTS (
    SELECT 1
...
-- 4) Bulk upsert RPC (inline + bulk updates)
...
BEGIN
  ...
  LOOP
    ...
    -- ensure category exists (brand_class + part_category unique)
