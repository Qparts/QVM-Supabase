-- Synced from QVM/test branch applied migration history (version 20260628223749, name: fix_purchase_invoices_per_item_join)
...
BEGIN
  ...
  -- Latest PO per confirmed_item (via purchase_items rows — procurement flow)
  -- Falls back to latest PO by confirmed_order if no purchase_items row exists
  latest_po_by_item AS (
    SELECT t.confirmed_item_id, t.purchase_order_id, t.vendor_invoice_url, t.vendor_invoice_number, t.zoho_bill_url, t.uploaded_by AS invoice_uploaded_by
    FROM (
      SELECT pi.confirmed_item_id, po.purchase_order_id, po.vendor_invoice_url, po.vendor_invoice_number, po.zoho_bill_url, po.uploaded_by,
  ...
  -- Latest PO per confirmed_order (fallback when no purchase_items row)
  ...
  -- Latest credit note per purchase_order
  latest_vcn_by_po AS (
    SELECT t.purchase_order_id, t.vendor_creditnote_url, t.uploaded_by AS creditnote_uploaded_by
    FROM (
      SELECT vcn.purchase_order_id, vcn.vendor_creditnote_url, vcn.uploaded_by,
             row_number() OVER (PARTITION BY vcn.purchase_order_id ORDER BY vcn.created_at DESC) AS rn
      FROM qvm_new_apps.vendor_creditnotes vcn
    ) t
    WHERE t.rn = 1
  ),
  -- All invoice attachments per purchase_order
  ...
  -- All credit note attachments per purchase_order
  vcn_attachments_per_po AS (
    SELECT purchase_order_id, COALESCE(array_agg(vendor_creditnote_url ORDER BY created_at DESC), ARRAY[]::text[]) AS vendor_creditnote_attachments
  ...
  base AS (
    SELECT
      ...
      qvi.vendor_id,
      vnd.vendor_name,
      -- Use item-level PO if it exists, otherwise fall back to order-level PO
      COALESCE(lpi.purchase_order_id, lpo.purchase_order_id) AS resolved_po_id,
      COALESCE(lpi.vendor_invoice_url, lpo.vendor_invoice_url) AS vendor_invoice_url,
      COALESCE(lpi.vendor_invoice_number, lpo.vendor_invoice_number) AS vendor_invoice_number,
      COALESCE(lpi.zoho_bill_url, lpo.zoho_bill_url) AS zoho_bill_url,
      COALESCE(lpi.invoice_uploaded_by, lpo.invoice_uploaded_by) AS invoice_uploaded_by,
    ...
    LEFT JOIN latest_po_by_item lpi ON lpi.confirmed_item_id = ci.confirmed_item_id
  ...
  base_with_attachments AS (
    SELECT
      b.*,
      ap.invoice_attachments,
      vap.vendor_creditnote_attachments,
      vcn.vendor_creditnote_url,
      vcn.creditnote_uploaded_by
    FROM base b
    LEFT JOIN attachments_per_po ap ON ap.purchase_order_id = b.resolved_po_id
    LEFT JOIN vcn_attachments_per_po vap ON vap.purchase_order_id = b.resolved_po_id
    LEFT JOIN latest_vcn_by_po vcn ON vcn.purchase_order_id = b.resolved_po_id
  ),
  filtered AS (
    SELECT * FROM base_with_attachments i
