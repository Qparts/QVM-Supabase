-- The Internal Users management page (/#/internal-users) was previously usable by any internal
-- user (user_type = 185), including branch-scoped "Internal Branch User" accounts, per an earlier
-- deliberate decision ("not gated on Qparts Admin specifically"). That decision is now reversed:
-- only Qparts Admin (unrestricted branch scope, i.e. get_internal_branch_scope() IS NULL) may list
-- or manage internal sub-users. This backs the frontend hiding the page/nav item from branch users.
