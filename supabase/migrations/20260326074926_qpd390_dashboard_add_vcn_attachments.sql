-- Synced from QVM/test branch applied migration history (version 20260326074926, name: qpd390_dashboard_add_vcn_attachments)
...
BEGIN
  ...
  WITH user_ctx AS (
    ...
    SELECT 
      ...
      vcn_latest.vendor_creditnote_url,
      (SELECT user_name FROM qvm_new_apps.user_data WHERE user_id = po.uploaded_by) AS invoice_uploaded_by_name,
      (SELECT user_name FROM qvm_new_apps.user_data WHERE user_id = vcn_latest.vcn_uploaded_by) AS creditnote_uploaded_by_name,
      COALESCE(pia.urls, '[]'::jsonb) AS invoice_attachments,
      COALESCE(vcn_list.urls, '[]'::jsonb) AS vendor_creditnote_attachments
    ...
    LEFT JOIN LATERAL (
      SELECT p.vendor_invoice_url, p.vendor_invoice_number, p.zoho_bill_url, p.purchase_order_id, p.uploaded_by
    ...
    LEFT JOIN LATERAL (
      SELECT v.vendor_creditnote_url, v.vendor_creditnote_number, v.uploaded_by AS vcn_uploaded_by, v.created_at
      FROM qvm_new_apps.vendor_creditnotes v
      WHERE v.purchase_order_id = po.purchase_order_id
      ORDER BY v.created_at DESC
      LIMIT 1
    ) vcn_latest ON true
    LEFT JOIN LATERAL (
      SELECT coalesce(jsonb_agg(pia.file_url ORDER BY pia.uploaded_at DESC), '[]'::jsonb) AS urls
      FROM qvm_new_apps.purchase_invoice_attachments pia
      WHERE pia.purchase_order_id = po.purchase_order_id
    ) pia ON true
    LEFT JOIN LATERAL (
      SELECT coalesce(jsonb_agg(v.vendor_creditnote_url ORDER BY v.created_at DESC), '[]'::jsonb) AS urls
      FROM qvm_new_apps.vendor_creditnotes v
      WHERE v.purchase_order_id = po.purchase_order_id
    ) vcn_list ON true
  ), filtered AS (
  ...
  FROM (
    SELECT 
      ...
      vcn_latest.vendor_creditnote_url AS vendor_creditnote_url,
      zoho_bill_url,
      branch_name,
      model,
      main_brand,
    ...
    LEFT JOIN LATERAL (
      SELECT p.uploaded_by FROM qvm_new_apps.purchase_orders p WHERE p.confirmed_order_id = f.confirmed_order_id ORDER BY p.created_at DESC LIMIT 1
    ) po ON true
    LEFT JOIN LATERAL (
      SELECT v.vendor_creditnote_url, v.uploaded_by AS vcn_uploaded_by
      FROM qvm_new_apps.vendor_creditnotes v
      WHERE v.purchase_order_id = (SELECT p.purchase_order_id FROM qvm_new_apps.purchase_orders p WHERE p.confirmed_order_id = f.confirmed_order_id ORDER BY p.created_at DESC LIMIT 1)
