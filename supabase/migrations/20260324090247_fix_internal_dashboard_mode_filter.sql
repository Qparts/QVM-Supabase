-- Synced from QVM/test branch applied migration history (version 20260324090247, name: fix_internal_dashboard_mode_filter)
-- Fix internal dashboard to respect p_mode parameter
-- 'regular' = RFQs only (not yet confirmed)
-- 'bulk' = Orders only (confirmed)
...
CREATE OR REPLACE FUNCTION public.get_internal_dashboard(
  ...
  p_account_managers uuid[] DEFAULT NULL::uuid[],
  p_clients integer[] DEFAULT NULL::integer[],
  p_branches integer[] DEFAULT NULL::integer[],
  p_brands integer[] DEFAULT NULL::integer[],
  p_statuses integer[] DEFAULT NULL::integer[],
  p_mode text DEFAULT 'regular'::text,
...
BEGIN
  ...
  WITH filtered_quotations AS (
    ...
    WHERE
      ...
      -- NEW: Filter by mode
      AND (
        (p_mode = 'regular' AND co.confirmed_order_id IS NULL)
        OR (p_mode = 'bulk' AND co.confirmed_order_id IS NOT NULL)
      )
      AND (
        p_search IS NULL
