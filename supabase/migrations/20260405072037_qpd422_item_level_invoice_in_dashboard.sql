-- Synced from QVM/test branch applied migration history (version 20260405072037, name: qpd422_item_level_invoice_in_dashboard)
-- QPD-422: Show invoice number per item (item-level join via purchase_items)
BEGIN;
...
BEGIN
  ...
  WITH user_ctx AS (
    ...
    LEFT JOIN LATERAL (
      ...
      JOIN qvm_new_apps.purchase_orders p ON p.purchase_order_id = pi.purchase_order_id
