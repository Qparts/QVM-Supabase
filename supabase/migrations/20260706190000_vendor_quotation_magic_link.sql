-- Unsecured, time-limited "magic link" for vendor RFQ pricing.
-- One opaque access_token per quotation_vendors row (vendor + branch + quotation), valid for
-- 7 days, refreshed on resend. Lets a vendor open the pricing page from an emailed link with no
-- Supabase Auth session, scoped tightly to just that row's items.
...
-- 2. create_vendors_quotations: return the token per vendor selection, refresh expiry on resend --
...
-- 4. get_vendor_emails: also resolve emails for the new multi-user vendor model ------------------
-- (previously only joined the legacy 1:1 vendors.user_id link, which is never set for logins
-- created via the vendor-branches/quick-add flows). Optional branch scoping: when given, prefers
-- admin-vendor(s) plus vendor-users assigned to that specific branch.
...
BEGIN
    ...
    FROM (
        ...
        -- New multi-user model: admin-vendor(s) always included
        SELECT u.email
        ...
        -- New multi-user model: branch-restricted vendor users, only for the given branch
        -- (or all of them when no branch was specified).
        SELECT u.email
        FROM qvm_new_apps.user_data u
        WHERE u.user_vendor = ANY(p_vendor_ids)
          AND u.email IS NOT NULL AND u.email <> ''
