-- Surface the vendor's own company name in get_vendor_quotation_details' response, so both the
-- authenticated vendor quotation page and the public magic-link page (which both call this same
-- function — the latter via get_vendor_quotation_by_token) can show "you are quoting as <vendor>"
-- without a separate round trip. Purely additive: identical query, one more top-level field.
