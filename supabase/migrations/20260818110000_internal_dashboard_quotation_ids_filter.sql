-- Actions tab table reuse: lets the frontend ask get_internal_dashboard for the exact set of
-- quotations that fell into an Actions bucket (ready_for_quotation / fully_priced / exceeded_sla /
-- confirmed_no_po), reusing the identical row-shaping the RFQs/Orders tabs already use instead of
-- building a second, divergent projection. Adding a parameter changes arity, so the old-arity
-- function is dropped first (same pattern as the insurance_company_id addition in
-- 20260726130000_quotations_insurance_company.sql).

DROP FUNCTION IF EXISTS public.get_internal_dashboard(uuid, text, timestamptz, timestamptz, uuid[], integer[], integer[], integer[], integer[], bigint[], text, text, integer, integer);
