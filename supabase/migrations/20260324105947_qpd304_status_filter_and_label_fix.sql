-- Synced from QVM/test branch applied migration history (version 20260324105947, name: qpd304_status_filter_and_label_fix)
CREATE OR REPLACE FUNCTION public.get_internal_dashboard(
  ...
  p_mode text DEFAULT 'regular', -- client type: 'regular' | 'bulk' | NULL (all)
...
BEGIN
  ...
  WITH filtered_quotations AS (
    ...
    WHERE
      ...
      AND (
        ...
        OR EXISTS (
          SELECT 1
          FROM qvm_new_apps.quotation_items qis2
          LEFT JOIN qvm_new_apps.confirmed_items cis2 ON cis2.quotation_item_id = qis2.quotation_item_id
          WHERE qis2.quotation_id = q.quotation_id
            AND COALESCE(cis2.item_status, qis2.item_status) = ANY(p_statuses)
        )
      )
  ),
  filtered_with_branch AS (
    SELECT
      ...
      ld_oc.list_data AS order_category_label,
      CASE WHEN ld_oc.list_data ILIKE 'Bulk' THEN true ELSE false END AS is_bulk_client
    ...
    LEFT JOIN qvm_new_apps.list_data ld_oc ON ld_oc.list_data_id = cb.order_category
    WHERE
      (p_branches IS NULL OR cb.customer_id = ANY(p_branches))
      AND (p_clients IS NULL OR cb.list_data_id = ANY(p_clients))
      AND (
        p_mode IS NULL
        OR (p_mode = 'bulk' AND (ld_oc.list_data ILIKE 'Bulk'))
        OR (p_mode = 'regular' AND ((ld_oc.list_data ILIKE 'Regular') OR ld_oc.list_data IS NULL))
      )
  ),
  ordered_rows AS (
    SELECT fwb.*
  ...
  row_objects AS (
    SELECT jsonb_build_object(
      ...
      'items', (
        ...
        LEFT JOIN qvm_new_apps.list_data ld_status ON ld_status.list_data_id = COALESCE(ci.item_status, qi.item_status)
        ...
        WHERE qi.quotation_id = fwb.quotation_id
          ...
          AND (p_statuses IS NULL OR COALESCE(ci.item_status, qi.item_status) = ANY(p_statuses))
      ),
      'quotation_notes', (
        SELECT jsonb_agg(
          jsonb_build_object(
