-- Adds a priced_parts count to get_vendor_quotations so the vendor Quotations
-- list can show real pricing progress (priced / total). Previously the RPC only
-- returned number_of_parts (total), so the progress bar was stuck at 0 / N.
--
-- A part counts as "priced" once its quotation_vendor_items row has a cost set
-- (cost IS NOT NULL AND cost > 0).
--
-- NOTE (status model): vendor_status is a single value per quotation_vendor, so
-- an order with some parts priced/confirmed and others still pending can only
-- show one status. The frontend uses (priced_parts < number_of_parts) to render
-- a "Partial" indicator. A fuller fix would track status per item for the vendor.
