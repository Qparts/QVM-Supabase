-- Fixes a false-positive "new price came back from vendor" (item_status -> 17) notification.
--
-- Root cause: create_vendors_quotations unconditionally force-set item_status = 237
-- ("Sent To Vendor") for every item in the RFQ, even when that item was already priced (17) or
-- further along (19 Confirmed, 21 Processing, 22 Out for Delivery, 23 Delivered, 31 Settled) from
-- an earlier vendor round — e.g. tendering the SAME item to one more vendor for a backup quote.
-- SendVendorRFQModal.tsx (handleSend) already recognized this was wrong and patched it
-- client-side immediately afterward, restoring item_status back to its pre-send value for any
-- item whose beforeStatus was >= 17. But that restore is a REAL item_status transition
-- (237 -> 17), which correctly fires the trg_dispatch_notification_rules_qi/log_activity triggers
-- for a transition TO 17 — even though no vendor actually priced anything; it's an artifact of
-- the force-down-then-restore round-trip, not a genuine new price.
--
-- Fix: never downgrade item_status to 237 for items already at a rank beyond it — mirrors the
-- exact status set services/apiService.ts's updateQuotationItemStatus rank guard already treats as
-- "further along than Sent To Vendor" (rank 4-9: 17,19,21,22,23,31). With this, an already-priced
-- item stays at 17 the whole time it's tendered to an additional vendor — no flicker, no restore
-- needed, no false notification. Only the item_status UPDATE (and its status_logs entry, now
-- limited to items actually changed) are touched; everything else is identical to the live
-- definition (qvm-supabase/supabase/migrations/20260708013510_fix_create_vendors_quotations_cascade_cost_logs.sql).
