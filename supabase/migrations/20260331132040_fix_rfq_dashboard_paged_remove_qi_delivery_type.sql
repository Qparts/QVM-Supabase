-- Synced from QVM/test branch applied migration history (version 20260331132040, name: fix_rfq_dashboard_paged_remove_qi_delivery_type)
...
BEGIN
  ...
  FROM (
    ...
    -- First item branch to derive client branch and company
    LEFT JOIN LATERAL (
    ...
    -- Items aggregation (uses quotation-level delivery/order labels)
    LEFT JOIN LATERAL (
      SELECT COALESCE(
        jsonb_agg(
          jsonb_build_object(
            ...
            'delivery_type', ld_delivery_q.list_data,
            'order_type', ld_order_q.list_data,
      ...
      LEFT JOIN qvm_new_apps.list_data ld_delivery_q ON ld_delivery_q.list_data_id = q.delivery_type
      LEFT JOIN qvm_new_apps.list_data ld_order_q ON ld_order_q.list_data_id = q.order_type
      WHERE qi.quotation_id = q.quotation_id
        AND (
          v_is_internal
          OR (
    ...
    WHERE status_info.has_filtered_status = true
  ) r;
...
GRANT EXECUTE ON FUNCTION public.rfq_dashboard_paged(uuid, text, text, text, text, uuid, timestamptz, timestamptz, int, int, text, text) TO anon, authenticated;;
