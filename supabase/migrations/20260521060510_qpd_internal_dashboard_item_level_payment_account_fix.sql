-- Synced from QVM/test branch applied migration history (version 20260521060510, name: qpd_internal_dashboard_item_level_payment_account_fix)
BEGIN;
...
DROP FUNCTION IF EXISTS public.update_purchase_account_inline(int, int);
...
CREATE OR REPLACE FUNCTION public.get_internal_dashboard(
  ...
  p_sort_by text DEFAULT 'rfqDate',
    ...
    'data', COALESCE(
      (
        SELECT jsonb_agg(
          page_rows.row_data
          ORDER BY
            CASE WHEN p_sort_by = 'rfqDate' AND p_sort_order = 'asc' THEN page_rows.rfq_date END ASC NULLS LAST,
            CASE WHEN p_sort_by = 'rfqDate' AND p_sort_order = 'desc' THEN page_rows.rfq_date END DESC NULLS LAST,
            CASE WHEN p_sort_by = 'orderNumber' AND p_sort_order = 'asc' THEN page_rows.order_number END ASC NULLS LAST,
            CASE WHEN p_sort_by = 'orderNumber' AND p_sort_order = 'desc' THEN page_rows.order_number END DESC NULLS LAST,
            CASE WHEN p_sort_by = 'clientName' AND p_sort_order = 'asc' THEN page_rows.client_name END ASC NULLS LAST,
            CASE WHEN p_sort_by = 'clientName' AND p_sort_order = 'desc' THEN page_rows.client_name END DESC NULLS LAST,
            page_rows.rfq_date DESC,
            page_rows.quotation_id DESC
        )
        FROM (
          SELECT
            fwb.quotation_id,
              ...
              'items', (
                SELECT jsonb_agg(
                  jsonb_build_object(
                    ...
                    'payment_account', po_item.payment_account,
                    'payment_account_id', po_item.payment_account_id,
                    'item_notes', (
                      SELECT jsonb_agg(
                        jsonb_build_object(
                          'note_id', n.note_id,
                ...
                LEFT JOIN LATERAL (
                  SELECT
                    ...
                    ld_payment_item.list_data AS payment_account,
                    po.payment_account AS payment_account_id
                  FROM qvm_new_apps.purchase_items pi
                  JOIN qvm_new_apps.purchase_orders po ON po.purchase_order_id = pi.purchase_order_id
                  LEFT JOIN qvm_new_apps.list_data ld_payment_item ON ld_payment_item.list_data_id = po.payment_account
                  WHERE pi.confirmed_item_id = ci.confirmed_item_id
                  ORDER BY po.created_at DESC NULLS LAST, pi.purchase_item_id DESC
                  LIMIT 1
                ) po_item ON true
          ...
          ORDER BY
            CASE WHEN p_sort_by = 'rfqDate' AND p_sort_order = 'asc' THEN fwb.rfq_date END ASC NULLS LAST,
            CASE WHEN p_sort_by = 'rfqDate' AND p_sort_order = 'desc' THEN fwb.rfq_date END DESC NULLS LAST,
            CASE WHEN p_sort_by = 'orderNumber' AND p_sort_order = 'asc' THEN fwb.order_number END ASC NULLS LAST,
            CASE WHEN p_sort_by = 'orderNumber' AND p_sort_order = 'desc' THEN fwb.order_number END DESC NULLS LAST,
            CASE WHEN p_sort_by = 'clientName' AND p_sort_order = 'asc' THEN fwb.client_name END ASC NULLS LAST,
            CASE WHEN p_sort_by = 'clientName' AND p_sort_order = 'desc' THEN fwb.client_name END DESC NULLS LAST,
            fwb.rfq_date DESC,
            fwb.quotation_id DESC
          LIMIT p_limit
          OFFSET p_offset
        ) page_rows
      ),
...
REVOKE EXECUTE ON FUNCTION public.get_internal_dashboard(
  uuid,
  text,
  timestamptz,
  timestamptz,
...
BEGIN
  ...
  JOIN purchase_orders po ON po.purchase_order_id = pi.purchase_order_id
  WHERE pi.confirmed_item_id = p_confirmed_item_id
  ORDER BY po.created_at DESC NULLS LAST, pi.purchase_item_id DESC
  LIMIT 1;

  IF v_purchase_order_id IS NULL THEN
    RAISE EXCEPTION 'purchase_order for confirmed_item_id % not found', p_confirmed_item_id;
  END IF;
  ...
  IF v_rows = 0 THEN
    RAISE EXCEPTION 'purchase_orders row for purchase_order_id % not found', v_purchase_order_id;
  END IF;
