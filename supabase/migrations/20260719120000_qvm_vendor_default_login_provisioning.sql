-- QNEW: every vendor should be able to log in with the default password 123456.
--
-- Vendors created via sheet import / admin vendor page / quick-add all insert only a company row
-- into qvm_new_apps.vendors (no login). This adds automatic login provisioning: an AFTER INSERT
-- trigger on vendors creates a proper GoTrue login (auth.users + auth.identities + user_data) with
-- the bcrypt-hashed default password 123456 for any vendor that has an email and no login yet.
--
-- NOTE ON GoTrue: manually-inserted auth.users rows must set the token columns
-- (confirmation_token, recovery_token, email_change, email_change_token_new, ...) to '' rather than
-- NULL — GoTrue scans them as non-null strings and login fails otherwise.
--
-- A one-time data fix was also run in production (not part of this migration, since it is a
-- non-idempotent data operation):
--   * created logins for the vendors that had an email but no login;
--   * reset every existing vendor login (user_type 205) to 123456.

CREATE OR REPLACE FUNCTION qvm_new_apps.ensure_vendor_login(p_vendor_id integer, p_password text DEFAULT '123456')
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'qvm_new_apps','auth','extensions','public'
AS $$
DECLARE
  v_email text;
  v_name  text;
  v_uid   uuid;
  v_role  integer;
BEGIN
  SELECT NULLIF(trim(email),''), vendor_name INTO v_email, v_name
  FROM qvm_new_apps.vendors WHERE vendor_id = p_vendor_id;
  IF v_email IS NULL THEN RETURN NULL; END IF;
  IF EXISTS (SELECT 1 FROM qvm_new_apps.user_data WHERE user_type = 205 AND user_vendor = p_vendor_id) THEN RETURN NULL; END IF;
  IF EXISTS (SELECT 1 FROM auth.users WHERE lower(email) = lower(v_email)) THEN RETURN NULL; END IF;

  SELECT ld.list_data_id INTO v_role
  FROM qvm_new_apps.list_data ld JOIN qvm_new_apps.lists l ON l.list_id = ld.list_id
  WHERE l.list_name = 'user_role' AND ld.list_data = 'Vendor Admin' LIMIT 1;
  IF v_role IS NULL THEN RETURN NULL; END IF;

  v_uid := gen_random_uuid();

  INSERT INTO auth.users (
    instance_id, id, aud, role, email, encrypted_password,
    email_confirmed_at, created_at, updated_at,
    raw_app_meta_data, raw_user_meta_data, is_sso_user, is_anonymous,
    confirmation_token, recovery_token, email_change, email_change_token_new,
    email_change_token_current, phone_change, phone_change_token, reauthentication_token
  ) VALUES (
    '00000000-0000-0000-0000-000000000000', v_uid, 'authenticated', 'authenticated',
    lower(v_email), extensions.crypt(p_password, extensions.gen_salt('bf', 10)),
    now(), now(), now(),
    '{"provider":"email","providers":["email"]}'::jsonb, '{}'::jsonb, false, false,
    '', '', '', '', '', '', '', ''
  );

  INSERT INTO auth.identities (provider_id, user_id, identity_data, provider, created_at, updated_at)
  VALUES (v_uid::text, v_uid,
    jsonb_build_object('sub', v_uid::text, 'email', lower(v_email), 'email_verified', true, 'phone_verified', false),
    'email', now(), now());

  INSERT INTO qvm_new_apps.user_data (user_id, email, user_name, user_type, user_role, user_vendor)
  VALUES (v_uid, lower(v_email), COALESCE(v_name, v_email), 205, v_role, p_vendor_id);

  RETURN v_uid;
END;
$$;

REVOKE ALL ON FUNCTION qvm_new_apps.ensure_vendor_login(integer, text) FROM PUBLIC;

-- Auto-provision a login for every newly created vendor (covers sheet import, admin vendor page,
-- quick-add from quotation, and any other path that inserts into vendors). Never blocks the insert.
CREATE OR REPLACE FUNCTION qvm_new_apps.trg_vendor_ensure_login()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'qvm_new_apps','auth','extensions','public'
AS $$
BEGIN
  BEGIN
    PERFORM qvm_new_apps.ensure_vendor_login(NEW.vendor_id, '123456');
  EXCEPTION WHEN OTHERS THEN
    RAISE WARNING 'ensure_vendor_login failed for vendor %: %', NEW.vendor_id, SQLERRM;
  END;
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS vendors_ensure_login ON qvm_new_apps.vendors;
CREATE TRIGGER vendors_ensure_login
AFTER INSERT ON qvm_new_apps.vendors
FOR EACH ROW EXECUTE FUNCTION qvm_new_apps.trg_vendor_ensure_login();
