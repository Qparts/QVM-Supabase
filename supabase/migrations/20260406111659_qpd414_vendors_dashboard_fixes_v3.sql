-- Synced from QVM/test branch applied migration history (version 20260406111659, name: qpd414_vendors_dashboard_fixes_v3)
BEGIN;

SET search_path TO qvm_new_apps, public;

-- Robust list_vendor_filters: guard against scalar JSONB values for brands/regions using CASE
...
BEGIN
  ...
  -- Brands: ensure array before expanding
  SELECT COALESCE(jsonb_agg(jsonb_build_object('id', name, 'name', name) ORDER BY name), '[]'::jsonb)
  INTO v_brands
  FROM (
    SELECT DISTINCT initcap(btrim(elem)) AS name
    FROM qvm_new_apps.vendors v,
         jsonb_array_elements_text(CASE WHEN jsonb_typeof(v.brands) = 'array' THEN v.brands ELSE '[]'::jsonb END) AS elem
  ) b;

  -- Regions: ensure array before expanding
  SELECT COALESCE(jsonb_agg(jsonb_build_object('id', name, 'name', name) ORDER BY name), '[]'::jsonb)
  INTO v_regions
  FROM (
    SELECT DISTINCT initcap(btrim(elem)) AS name
    FROM qvm_new_apps.vendors v,
         jsonb_array_elements_text(CASE WHEN jsonb_typeof(v.region) = 'array' THEN v.region ELSE '[]'::jsonb END) AS elem
  ) r;
...
-- Fix get_vendors_dashboard: single statement computes total and rows; guard brand/region JSON array usage
...
BEGIN
  ...
  SELECT c.c,
         (SELECT COALESCE(jsonb_agg(to_jsonb(p) ORDER BY p.vendor_name), '[]'::jsonb) FROM paged p)
  INTO v_total, v_rows
  FROM cnt c;
