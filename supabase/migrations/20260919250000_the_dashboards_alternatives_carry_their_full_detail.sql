-- The dashboard's alternatives carry their full detail.
--
-- Origin, quantity, delivery days, note and photos join number, grade, brand and price, so the
-- Alt Part Number column shows an alternative as the vendor's pricing page does.
...
CREATE OR REPLACE FUNCTION public.approval_flow_version()
 RETURNS integer LANGUAGE sql STABLE AS $$ SELECT 29 $$;
