...
begin
  ...
  with cc as (
    select cost_range_id, cost_range,
      (regexp_replace(split_part(cost_range, ',', 1), '[^0-9.]', '', 'g'))::numeric AS lo,
      (regexp_replace(split_part(cost_range, ',', 2), '[^0-9.]', '', 'g'))::numeric AS hi
  ...
  select coalesce(jsonb_agg(jsonb_build_object('range', g.cost_range, 'cost_range_id', g.crid,
      'actual_pct', g.actual_pct, 'target_pct', g.target_pct, 'items', g.n) order by g.crid), '[]'::jsonb)
    into v_margin
  from (
    select crid, cost_range,
      ...
      count(*) AS n
    from withtarget group by crid, cost_range
  ) g;
  ...
  select coalesce(jsonb_agg(jsonb_build_object('branch', cb.branch_name, 'target', bt.target,
      ...
      'pct', case when bt.target>0 then round(100.0*coalesce(s.actual,0)/bt.target,1) else null end) order by bt.target desc), '[]'::jsonb)
    into v_branch
