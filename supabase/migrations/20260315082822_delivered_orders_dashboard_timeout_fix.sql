-- Synced from QVM/test branch applied migration history (version 20260315082822, name: delivered_orders_dashboard_timeout_fix)
...
BEGIN
  ...
  WITH dn_base AS (
    SELECT DISTINCT ON (d.delivery_id, cb.customer_id)
      ...
      d.shipping_price,
      (
        SELECT COALESCE(SUM(qi3.price_before_vat * di3.delivered_qty), 0)
        FROM qvm_new_apps.delivery_items di3
        JOIN qvm_new_apps.confirmed_items ci3 ON ci3.confirmed_item_id = di3.confirmed_item_id
        JOIN qvm_new_apps.quotation_items qi3 ON qi3.quotation_item_id = ci3.quotation_item_id
        WHERE di3.delivery_id = d.delivery_id
          AND qi3.customer_id = cb.customer_id
      ) AS total_before_vat,
      (
        SELECT COALESCE(SUM(qi3.price_before_vat * di3.delivered_qty * 1.15), 0)
        FROM qvm_new_apps.delivery_items di3
        JOIN qvm_new_apps.confirmed_items ci3 ON ci3.confirmed_item_id = di3.confirmed_item_id
        JOIN qvm_new_apps.quotation_items qi3 ON qi3.quotation_item_id = ci3.quotation_item_id
        WHERE di3.delivery_id = d.delivery_id
      ...
      (
        SELECT COALESCE(SUM(qi3.price_before_vat * di3.delivered_qty * 0.15), 0)
        FROM qvm_new_apps.delivery_items di3
        JOIN qvm_new_apps.confirmed_items ci3 ON ci3.confirmed_item_id = di3.confirmed_item_id
        JOIN qvm_new_apps.quotation_items qi3 ON qi3.quotation_item_id = ci3.quotation_item_id
        WHERE di3.delivery_id = d.delivery_id
      ...
      EXISTS (
        ...
        JOIN qvm_new_apps.confirmed_items ci4 ON ci4.confirmed_item_id = di4.confirmed_item_id
        JOIN qvm_new_apps.quotation_items qi4 ON qi4.quotation_item_id = ci4.quotation_item_id
        WHERE di4.delivery_id = d.delivery_id
          AND qi4.customer_id = cb.customer_id
  ...
  rn_base AS (
    SELECT DISTINCT ON (r.return_id, cb.customer_id)
      ...
      r.shipping_price,
      (
        SELECT COALESCE(SUM(qi3.price_before_vat * ri3.return_qty), 0)
      ...
      (
        SELECT COALESCE(SUM(qi3.price_before_vat * ri3.return_qty * 1.15), 0)
        FROM qvm_new_apps.return_items ri3
        JOIN qvm_new_apps.confirmed_items ci3 ON ci3.confirmed_item_id = ri3.confirmed_item_id
        JOIN qvm_new_apps.quotation_items qi3 ON qi3.quotation_item_id = ci3.quotation_item_id
        WHERE ri3.return_id = r.return_id
      ...
      (
        SELECT COALESCE(SUM(qi3.price_before_vat * ri3.return_qty * 0.15), 0)
        FROM qvm_new_apps.return_items ri3
        JOIN qvm_new_apps.confirmed_items ci3 ON ci3.confirmed_item_id = ri3.confirmed_item_id
        JOIN qvm_new_apps.quotation_items qi3 ON qi3.quotation_item_id = ci3.quotation_item_id
        WHERE ri3.return_id = r.return_id
      ...
      EXISTS (
        ...
        JOIN qvm_new_apps.confirmed_items ci4 ON ci4.confirmed_item_id = ri4.confirmed_item_id
        JOIN qvm_new_apps.quotation_items qi4 ON qi4.quotation_item_id = ci4.quotation_item_id
