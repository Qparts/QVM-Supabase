-- Synced from QVM/test branch applied migration history (version 20260426011135, name: qpd413_fix_add_rfq_item_inline_defaults_and_line_code)
BEGIN;

-- QPD-413 (fix): add_rfq_item_inline with proper defaults ordering and line_item_code generation
...
BEGIN
  ...
  SELECT COALESCE(
    MAX(
      NULLIF(regexp_replace(COALESCE(line_item_code, ''), '^.*-(\\d+)$', '\\1'), '')::int
    ), 0
  ) + 1
  INTO v_next_index
  ...
  -- Optional: attach initial note if provided
