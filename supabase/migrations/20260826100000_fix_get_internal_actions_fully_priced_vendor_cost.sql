-- Fix: "Fully priced" was checking qi.item_status = 17 (Priced) — a manually-set status staff
-- toggle by hand — which doesn't reliably reflect whether a vendor price actually exists yet.
-- Correct rule (per business): an order is fully priced when every one of its items has at least
-- one vendor quotation row (qvm_new_apps.quotation_vendor_items) with a cost actually set.
