-- Synced from QVM/test branch applied migration history (version 20260524122545, name: change_item_status_bulk_delivery_return_side_effects)
  ...
  if lower(trim(v_new_status_label)) = 'delivered' then
    ...
    inserted_deliveries as (
      ...
      where not exists (
        select 1
        from deliveries d
        where d.confirmed_order_id = tc.confirmed_order_id
      )
    ...
    join lateral (
      ...
      from deliveries d
      where d.confirmed_order_id = tc.confirmed_order_id
      order by d.created_at asc nulls last, d.delivery_id asc
      limit 1
    ) d on true
    where not exists (
      select 1
    ...
    inserted_returns as (
      ...
      where not exists (
        select 1
        from returns r
        where r.confirmed_order_id = tc.confirmed_order_id
      )
    ...
    join lateral (
      ...
      from returns r
      where r.confirmed_order_id = tc.confirmed_order_id
      order by r.created_at asc nulls last, r.return_id asc
      limit 1
    ) r on true
    where not exists (
      select 1
