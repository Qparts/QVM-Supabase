...
-- _uk index is dropped and the _key constraint stays.
--
-- Function bodies are the live ones on QVM/test with only the lock added.
...
CREATE OR REPLACE FUNCTION public.add_quotation_item_by_vendor(p_quotation_id integer, p_part_number text DEFAULT NULL::text, p_part_description text DEFAULT NULL::text, p_quantity integer DEFAULT 1, p_brand_class integer DEFAULT NULL::integer, p_cost numeric DEFAULT NULL::numeric, p_note text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
