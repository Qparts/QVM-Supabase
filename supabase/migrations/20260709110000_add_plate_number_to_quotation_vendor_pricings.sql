-- Surface the order's plate number in get_quotation_vendor_pricings — the Pricing page needs it
-- to include plate_number in the n8n webhook fired when a purchase order is created (it already
-- joins qvm_new_apps.quotations for shipping_price, so this is just one more column from that
-- same join, not a new join).
