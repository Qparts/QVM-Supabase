-- The last unscoped predicate: the items pane of the confirmed-orders dashboard.
--
-- The order list was scoped and its item rows were not, so an order visible for one branch showed
-- its lines from every other branch too. An order can genuinely hold lines from several branches,
-- which is why this is a filter rather than an oversight to delete.
...
BEGIN
  ...
  items_cte AS (
    ...
    WHERE ci.confirmed_order_id IN (SELECT confirmed_order_id FROM paged_orders)
      AND (
        -- The items pane of an order the account can see: an order can hold lines from more than
        -- one branch, and only the ones in scope belong on the screen.
        (v_is_internal AND (v_scope IS NULL OR qi.customer_id = ANY(v_scope)))
        OR (
          v_user_role = 170
          AND EXISTS (
