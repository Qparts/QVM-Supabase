-- Adds a nullable p_branch_id filter (internal users only; external users already forced to
-- their own branch) to get_confirmed_orders_dashboard and rfq_dashboard_paged, matching the
-- existing convention used by get_delivered_orders_dashboard / get_purchase_invoices_dashboard /
-- get_return_exchange_dashboard / get_archive_note_rows. Omitting the parameter (or passing
-- NULL) preserves today's behavior exactly - internal users see every branch.
--
...
REVOKE EXECUTE ON FUNCTION public.get_confirmed_orders_dashboard(uuid, text, text, text, text, uuid, timestamp with time zone, timestamp with time zone, integer, integer, text, text, integer) FROM public, anon;
GRANT EXECUTE ON FUNCTION public.get_confirmed_orders_dashboard(uuid, text, text, text, text, uuid, timestamp with time zone, timestamp with time zone, integer, integer, text, text, integer) TO authenticated;

-- ============================================================================================

DROP FUNCTION IF EXISTS public.rfq_dashboard_paged(
  uuid, text, text, text, text, uuid, timestamptz, timestamptz, int, int, text, text
);

CREATE FUNCTION public.rfq_dashboard_paged(
  p_user_id uuid,
...
REVOKE EXECUTE ON FUNCTION public.rfq_dashboard_paged(uuid, text, text, text, text, uuid, timestamptz, timestamptz, int, int, text, text, integer) FROM public, anon;
GRANT EXECUTE ON FUNCTION public.rfq_dashboard_paged(uuid, text, text, text, text, uuid, timestamptz, timestamptz, int, int, text, text, integer) TO authenticated;
