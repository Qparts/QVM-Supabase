-- Synced from QVM/test branch applied migration history (version 20260624020826, name: refix_delivery_notes_discount_from_quotation_items)

-- Re-backfill delivery_notes: use actual qi.discount_percent (no coalesce to 0)
-- and recalculate totals where the stored discount differs from quotation_items
...
SET
  ...
  total_price_before_vat = round(
    (coalesce(dn.price_before_vat, 0)
     * coalesce(dn.approved_quantity, 1)
     * (1 - coalesce(qi.discount_percent, 0) / 100.0))::numeric, 2
  ),
  vat = round(
    (coalesce(dn.price_before_vat, 0)
     * coalesce(dn.approved_quantity, 1)
     * (1 - coalesce(qi.discount_percent, 0) / 100.0)
     * 0.15)::numeric, 2
  ),
...
-- Same for return_notes
UPDATE qvm_new_apps.return_notes rn
SET
  discount_percent = coalesce(qi.discount_percent, 0),
  shipping_price   = coalesce(q.shipping_price, 0),
  total_price_before_vat = round(
    (coalesce(rn.price_before_vat, 0)
     * coalesce(rn.return_quantity, 1)
     * (1 - coalesce(qi.discount_percent, 0) / 100.0))::numeric, 2
  ),
  vat = round(
    (coalesce(rn.price_before_vat, 0)
