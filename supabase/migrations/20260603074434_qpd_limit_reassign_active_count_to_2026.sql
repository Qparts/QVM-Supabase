-- Synced from QVM/test branch applied migration history (version 20260603074434, name: qpd_limit_reassign_active_count_to_2026)
BEGIN;
...
DECLARE
  ...
  v_active_count_from timestamptz := '2026-01-01 00:00:00+00'::timestamptz;
  v_active_count_to timestamptz := '2027-01-01 00:00:00+00'::timestamptz;
...
BEGIN
  ...
  LOOP
    ...
    SELECT
      CAST(MAX(CASE WHEN amb.slot_number = v_selected_slot THEN (amb.main_account_manager)::text END) AS uuid),
      CAST(MAX(CASE WHEN amb.slot_number = v_selected_slot THEN (amb.first_substitute)::text END) AS uuid),
      CAST(MAX(CASE WHEN amb.slot_number = v_selected_slot THEN (amb.second_substitute)::text END) AS uuid)
    ...
    LOOP
      ...
      WHERE q.account_manager = v_candidate
        AND q.created_at >= v_active_count_from
        AND q.created_at < v_active_count_to
        AND EXISTS (
          SELECT 1
          FROM qvm_new_apps.quotation_items qi
          LEFT JOIN qvm_new_apps.confirmed_items ci
