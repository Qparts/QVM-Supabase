-- Fix: purchase_items.approved_qty (partially populated) and .final_purchase_price (never
-- populated, confirmed 0/21 rows live) are not reliable sources. The real values live on
-- confirmed_items.approved_qty and quotation_vendor_items.cost (via quotation_items.cost_id),
-- exactly as get_purchase_invoices_dashboard already relies on. Both receiving RPCs were reading
-- the wrong columns, showing "Value 0.00" and "Qty: 0" everywhere.
