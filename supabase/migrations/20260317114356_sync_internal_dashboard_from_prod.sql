-- Synced from QVM/test branch applied migration history (version 20260317114356, name: sync_internal_dashboard_from_prod)
BEGIN;

-- Apply production definition of get_internal_dashboard
    ...
    SELECT jsonb_build_object(
      ...
      'items', (
        ...
        WHERE qi.quotation_id = fwb.quotation_id
          ...
          AND (
            p_search IS NULL
            OR qi.part_number ILIKE concat('%', p_search, '%')
            OR qi.part_description ILIKE concat('%', p_search, '%')
            OR qi.vin ILIKE concat('%', p_search, '%')
            OR ci.final_part_number ILIKE concat('%', p_search, '%')
          )
      ),
      'quotation_notes', (
        SELECT jsonb_agg(
...
REVOKE ALL ON FUNCTION public.get_internal_dashboard(uuid, text, timestamptz, timestamptz, uuid[], int[], int[], int[], int[], text, int, int) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.get_internal_dashboard(uuid, text, timestamptz, timestamptz, uuid[], int[], int[], int[], int[], text, int, int) FROM anon;
...
-- Apply production definition of get_account_managers_list
CREATE OR REPLACE FUNCTION public.get_account_managers_list(p_user_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
...
BEGIN
  ...
  RETURN (
    ...
    WHERE ud.user_type = 185 AND ud.user_id IS NOT NULL
  );
END;
$function$;
