-- Synced from QVM/test branch applied migration history (version 20260405115139, name: qpd425_dashboard_include_attachments)
-- QPD-425: Include invoice and creditnote attachments per item in dashboard
BEGIN;
...
BEGIN
  ...
  user_ctx AS (
    ...
    SELECT t.confirmed_item_id, t.purchase_order_id, t.vendor_invoice_url, t.vendor_invoice_number, t.zoho_bill_url,
           ap.invoice_attachments, vap.vendor_creditnote_attachments
    FROM (
    ...
    LEFT JOIN attachments_per_po ap ON ap.purchase_order_id = t.purchase_order_id
    LEFT JOIN vcn_attachments_per_po vap ON vap.purchase_order_id = t.purchase_order_id
    WHERE t.rn = 1
  ), latest_delivered AS (
    ...
    SELECT 
      ...
      lpo.zoho_bill_url,
      lpo.invoice_attachments,
      lpo.vendor_creditnote_attachments,
