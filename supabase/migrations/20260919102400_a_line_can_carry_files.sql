-- A vendor can attach files to the line they are pricing.
--
-- The order carried vendor attachments as a whole — an invoice, a quote PDF — and an alternative
-- carried its photos, but the line itself, the part being priced, could carry nothing: no photo of
-- the part on the shelf, no datasheet, no scan of the box. Now it can. Same shape as the
-- alternative's photos — a jsonb list of {url, path, name, type} in the public attachments bucket —
-- and written through the same bulk save as the prices, the whole list each time, so a removed file
-- stays removed.
