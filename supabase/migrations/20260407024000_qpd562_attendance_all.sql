...
AS $$
DECLARE
  b integer := extract(dow from base);
...
BEGIN
  ...
  IF v_has_vacation OR v_has_excuse THEN RETURN false; END IF;
  IF v_has_overtime THEN RETURN true; END IF;
  RETURN v_day_ok;
END;
$$;
...
BEGIN
  ...
  SELECT EXISTS (
    ...
    WHERE u.user_id = v_manager AND lower(r.list_data) IN ('qparts account manager','account manager')
  ) INTO v_is_am;
  ...
  IF current_date BETWEEN v_start AND v_end THEN PERFORM public.apply_attendance_for_today(); END IF;
