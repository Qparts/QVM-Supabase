-- Add Missing-PI / Missing-RN filters to the PO-level dashboard, matching the semantics already
-- used by get_purchase_invoices_dashboard's item-level equivalent, but evaluated at the PO's own
-- invoice/credit-note fields (both live on purchase_orders / vendor_creditnotes already).
DROP FUNCTION IF EXISTS qvm_new_apps.get_purchase_orders_receipt_dashboard(uuid, boolean, text, int[], int[], int, int);
  ...
  SELECT jsonb_build_object(
    ...
    'rows', COALESCE((
      SELECT jsonb_agg(to_jsonb(t) ORDER BY t.confirmation_date DESC) FROM (
        ...
        FROM filtered ORDER BY confirmation_date DESC LIMIT p_limit OFFSET p_offset
      ) t
    ), '[]'::jsonb)
  ) INTO v_result;
...
GRANT EXECUTE ON FUNCTION qvm_new_apps.get_purchase_orders_receipt_dashboard(uuid, boolean, text, int[], int[], int, int, boolean, boolean) TO authenticated;
