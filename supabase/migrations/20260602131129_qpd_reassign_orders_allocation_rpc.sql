-- Synced from QVM/test branch applied migration history (version 20260602131129, name: qpd_reassign_orders_allocation_rpc)
BEGIN;

SET search_path TO qvm_new_apps, public;

-- QPD: Reassign orders using allocation + substitute + capacity logic.
-- Notes:
-- 1. We keep the original reassign_orders_rpc intact and add a new RPC.
-- 2. Slot timing is not modeled elsewhere in the schema, so the caller provides
--    the current slot and can override the handover cutoff/timezone when needed.
-- 3. Capacity is enforced against active quotations before assignment and also
--    within the current batch so we do not overfill one manager mid-loop.
...
BEGIN
  ...
  FOR v_quotation IN
    SELECT q.quotation_id, q.branch_id, q.account_manager
...
REVOKE EXECUTE ON FUNCTION public.reassign_orders_allocation_rpc(integer[], text, smallint, timestamptz, time, text, integer) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.reassign_orders_allocation_rpc(integer[], text, smallint, timestamptz, time, text, integer) TO authenticated;
GRANT EXECUTE ON FUNCTION public.reassign_orders_allocation_rpc(integer[], text, smallint, timestamptz, time, text, integer) TO service_role;
