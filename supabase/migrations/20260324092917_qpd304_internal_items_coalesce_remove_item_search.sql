-- Synced from QVM/test branch applied migration history (version 20260324092917, name: qpd304_internal_items_coalesce_remove_item_search)
-- Improve get_internal_dashboard: ensure items never null and do not filter items by p_search
    ...
    SELECT jsonb_build_object(
      ...
      'items', (
        ...
        WHERE qi.quotation_id = fwb.quotation_id
          ...
          -- IMPORTANT: do not filter items by p_search here; header search already handled above
      ),
      'quotation_notes', (
        SELECT jsonb_agg(
          jsonb_build_object(
