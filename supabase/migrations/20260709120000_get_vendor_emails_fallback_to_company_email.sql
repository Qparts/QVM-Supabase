-- 607 of 610 vendor companies have no login at all, and none of them have the legacy
-- vendors.user_id link either — so get_vendor_emails resolved to zero recipients for any of them,
-- meaning an RFQ sent to a login-less vendor silently never reached anyone. 30 of those 607 do have
-- a plain vendors.email contact address on file; fall back to it so those at least get notified.
