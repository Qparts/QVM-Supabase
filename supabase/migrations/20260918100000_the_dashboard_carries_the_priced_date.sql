-- The dashboard carries the date an order was priced.
--
-- The by-order view shows when an order was created and when it was confirmed, and the gap between
-- them is mostly pricing — so the one date missing from the row is the one that explains the other
-- two. quotations has no priced_at column, and adding one would leave every order priced before
-- today with a blank where a date belongs.
--
-- status_logs already holds it. The earliest move to 17 on any item of the order is the moment that
-- order stopped waiting to be priced, which is what the column means.
      ...
      (
        SELECT jsonb_agg(
          jsonb_build_object(
            ...
            -- When this order was first priced. status_logs records the transition, so the earliest
            -- move to 17 on any of its items is the moment the order stopped waiting on pricing.
            -- Derived rather than stored: quotations has no priced_at, and adding one would leave
            -- every order that was priced before today with a blank where a date belongs.
            'priced_date', (
              SELECT min(sl.created_at)
                FROM qvm_new_apps.status_logs sl
                JOIN qvm_new_apps.quotation_items qi2 ON qi2.quotation_item_id = sl.quotation_item_id
               WHERE qi2.quotation_id = fwb.quotation_id AND sl.item_status = 17),
