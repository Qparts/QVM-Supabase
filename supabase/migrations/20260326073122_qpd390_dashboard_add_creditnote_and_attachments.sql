-- Synced from QVM/test branch applied migration history (version 20260326073122, name: qpd390_dashboard_add_creditnote_and_attachments)
...
BEGIN
  ...
  WITH user_ctx AS (
    ...
    SELECT 
      ...
      (SELECT user_name FROM qvm_new_apps.user_data WHERE user_id = vcn.vcn_uploaded_by) AS creditnote_uploaded_by_name,
    ...
    LEFT JOIN LATERAL (
      SELECT v.vendor_creditnote_url, v.vendor_creditnote_number, v.uploaded_by AS vcn_uploaded_by
    ...
    LEFT JOIN LATERAL (
      SELECT coalesce(jsonb_agg(pia.file_url), '[]'::jsonb) AS urls
  ...
  FROM (
    SELECT 
      ...
      vcn.vendor_creditnote_url AS vendor_creditnote_url,
      zoho_bill_url,
      branch_name,
      model,
      main_brand,
