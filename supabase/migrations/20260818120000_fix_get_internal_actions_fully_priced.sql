-- Fix: the "Fully priced" Actions bucket only checked items that had reached item_status = 237
-- (Sent To Vendor) and had a vendor cost recorded — it ignored items still sitting in an earlier
-- status (e.g. Ready For Quotation), so an order with some items never even sent to a vendor could
-- still show up as "fully priced" as long as the items that *were* sent all had a price back.
-- Correct rule: every item on the order must actually be in item_status = 17 (Priced) — the
-- catalog's real "Priced" status, distinct from 237 "Sent To Vendor".
...
begin
  ...
  from (
    select q.quotation_id, q.order_number, q.created_at as rfq_date,
           coalesce(cb.branch_name, '') as branch_name,
           count(*) as sent_count,
           count(*) filter (where qi.item_status = 17) as priced_count
    from qvm_new_apps.quotation_items qi
    join qvm_new_apps.quotations q on q.quotation_id = qi.quotation_id
    left join qvm_new_apps.client_branches cb on cb.customer_id = qi.customer_id
    group by q.quotation_id, q.order_number, q.created_at, cb.branch_name
    having count(*) > 0
       and count(*) = count(*) filter (where qi.item_status = 17)
  ) r;
