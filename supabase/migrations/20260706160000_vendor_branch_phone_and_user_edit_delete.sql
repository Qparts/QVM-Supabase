...
-- 2. list_vendor_users: no change needed to signature, but recreate to keep in sync with
--    schema comments / consistent ordering is unnecessary — the existing function already
--    reads whatever columns it needs from user_data, so it is left as-is here.
