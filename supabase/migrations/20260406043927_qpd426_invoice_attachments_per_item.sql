-- Synced from QVM/test branch applied migration history (version 20260406043927, name: qpd426_invoice_attachments_per_item)
-- QPD-426: Dashboard should show all PI attachments per item (across all POs), not just latest PO
BEGIN;
...
BEGIN
  ...
  user_ctx AS (
    ...
    -- Unique attachment URLs per item across ALL POs, ordered by last upload time desc
    WITH att AS (
      SELECT pi.confirmed_item_id, pia.file_url, max(pia.uploaded_at) AS last_uploaded_at
      FROM qvm_new_apps.purchase_items pi
      JOIN qvm_new_apps.purchase_invoice_attachments pia ON pia.purchase_order_id = pi.purchase_order_id
      GROUP BY pi.confirmed_item_id, pia.file_url
    )
    SELECT confirmed_item_id, array_agg(file_url ORDER BY last_uploaded_at DESC) AS invoice_attachments
    FROM att
    GROUP BY confirmed_item_id
  ), vcn_per_item AS (
    WITH att AS (
      SELECT pi.confirmed_item_id, v.vendor_creditnote_url AS file_url, max(v.uploaded_at) AS last_uploaded_at
      FROM qvm_new_apps.purchase_items pi
      JOIN qvm_new_apps.vendor_creditnotes v ON v.purchase_order_id = pi.purchase_order_id
      GROUP BY pi.confirmed_item_id, v.vendor_creditnote_url
    )
    SELECT confirmed_item_id, array_agg(file_url ORDER BY last_uploaded_at DESC) AS vendor_creditnote_attachments
    FROM att
    GROUP BY confirmed_item_id
  ), latest_delivered AS (
    ...
    SELECT 
      ...
      vp.vendor_creditnote_attachments,
    ...
    LEFT JOIN attachments_per_item ap ON ap.confirmed_item_id = ci.confirmed_item_id
    LEFT JOIN vcn_per_item vp ON vp.confirmed_item_id = ci.confirmed_item_id
    ...
    WHERE (s = '' OR position(lower(s) in lower(coalesce(i.order_number, ''))) > 0 OR position(lower(s) in lower(coalesce(i.vendor_invoice_number, ''))) > 0 OR position(lower(s) in lower(coalesce(i.vendor_name, ''))) > 0 OR position(lower(s) in lower(coalesce(i.final_part_number, ''))) > 0)
      AND (NOT p_missing_pi OR i.confirmed_order_id IN (
