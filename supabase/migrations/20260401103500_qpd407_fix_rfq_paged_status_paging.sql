-- Fix rfq_dashboard_paged to apply status filter BEFORE pagination and total_count
-- This ensures clicking summary widgets (e.g., ?status=15 for New RFQs) returns data reliably
  ...
  FROM (
    ...
    LEFT JOIN LATERAL (
      SELECT 
        ...
        -- keep this for compatibility, but final filter is now in candidates
        CASE 
    ...
    LEFT JOIN LATERAL (
      ...
      LEFT JOIN qvm_new_apps.list_data ld_delivery2 ON ld_delivery2.list_data_id = qi.delivery_type
      LEFT JOIN qvm_new_apps.list_data ld_order2 ON ld_order2.list_data_id = qi.order_type
    ...
    WHERE TRUE -- status filter already applied in candidates
  ) r;
...
-- Ask PostgREST to reload schema to pick up function changes immediately (if supported)
