-- Synced from QVM/test branch applied migration history (version 20260325132124, name: qpd385_fix_integer_equals_array_any_coalesce)
-- QPD-385: Fix integer = integer[] error by coercing ANY arrays explicitly
  ...
  IF p_missing_rn THEN
    WITH delivered_ids AS (
      ...
      LEFT JOIN LATERAL (
        ...
        WHERE sl.confirmed_item_id = ci.confirmed_item_id
          AND sl.item_status = ANY (COALESCE((SELECT ids FROM delivered_ids), ARRAY[]::int[]))
      ...
      WHERE
        ...
        AND (i.purchase_order_id IS NULL OR NOT EXISTS (
          SELECT 1 FROM vendor_creditnotes c WHERE c.purchase_order_id = i.purchase_order_id
        ))
    )
    ...
    FROM (
      SELECT 
        ...
        NULL::text AS vendor_creditnote_url,
        branch_name,
        model,
        main_brand,
        part_description,
