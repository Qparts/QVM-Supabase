-- An answered line shows the choice standing on it now.
--
-- For a line whose request was already answered, the view reported the choice recorded on that
-- old round, so a pick the workshop made afterwards looked undone on the next open. The open
-- request's record is the choice while it is open; otherwise the vendor line's, which is what the
-- order is actually written from.
