-- Synced from QVM/test branch applied migration history (version 20260628230558, name: fix_upsert_purchase_order_items_always_new_po)
-- Always create a fresh purchase_order on each upload (both add and replace modes)
-- so that invoice number/url updates never bleed across items.
