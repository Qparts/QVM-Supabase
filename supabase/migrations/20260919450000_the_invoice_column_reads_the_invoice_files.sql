-- A purchase order's Invoice column reads the invoice files, not only the fields on its row.
--
-- The upload path files an invoice in purchase_invoice_attachments and never wrote a URL on the
-- purchase order, so a PO with invoices on file read "No invoice"; older uploads even filed them on a
-- holder purchase order of their own. Both the PO list and the item rows now count the files filed
-- on the PO or on the order's holders, and the item rows stop borrowing another PO's invoice.
...
CREATE OR REPLACE FUNCTION public.approval_flow_version()
 RETURNS integer LANGUAGE sql STABLE AS $$ SELECT 49 $$;
