...
-- Robust list_vendor_filters: guard against scalar JSONB values for brands/regions
...
BEGIN
  ...
  -- Brands: only expand arrays to avoid scalar extraction errors
  SELECT COALESCE(jsonb_agg(jsonb_build_object('id', name, 'name', name) ORDER BY name), '[]'::jsonb)
  INTO v_brands
  FROM (
    SELECT DISTINCT initcap(btrim(elem)) AS name
    FROM qvm_new_apps.vendors v
    WHERE jsonb_typeof(v.brands) = 'array'
    CROSS JOIN LATERAL jsonb_array_elements_text(v.brands) AS elem
  ) b;

  -- Regions: only expand arrays to avoid scalar extraction errors
  SELECT COALESCE(jsonb_agg(jsonb_build_object('id', name, 'name', name) ORDER BY name), '[]'::jsonb)
  INTO v_regions
  FROM (
    SELECT DISTINCT initcap(btrim(elem)) AS name
    FROM qvm_new_apps.vendors v
    WHERE jsonb_typeof(v.region) = 'array'
    CROSS JOIN LATERAL jsonb_array_elements_text(v.region) AS elem
  ) r;
...
-- Fix get_vendors_dashboard: compute total and rows in one SELECT so CTE scope is valid
...
BEGIN
  ...
  SELECT c.c AS total, s.rows
  INTO v_total, v_rows
  FROM cnt c
  CROSS JOIN LATERAL (
    SELECT COALESCE(jsonb_agg(to_jsonb(p) ORDER BY p.vendor_name), '[]'::jsonb) AS rows
    FROM paged p
  ) s;
