...
alter table qvm_new_apps.quotation_items add column if not exists extraction_status text;

do $$ begin
  if not exists (select 1 from pg_constraint where conname = 'quotation_items_extraction_status_chk') then
    alter table qvm_new_apps.quotation_items add constraint quotation_items_extraction_status_chk
      check (extraction_status is null or extraction_status in ('cannot_extract','unclear'));
  end if;
end $$;
...
begin
  ...
  select count(*) filter (where is_manual),
