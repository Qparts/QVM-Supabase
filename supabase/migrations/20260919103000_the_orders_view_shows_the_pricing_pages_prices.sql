-- The order's view shows the pricing page's prices, and the wholesale one only to the workshop.
--
-- Two corrections. A line not yet sent for approval showed the raw vendor offer; it now shows what
-- the pricing page's تسعيرك column holds — سعر الجملة to the workshop, سعر العميل to the customer —
-- and the customer reference read price_before_vat, which is the wholesale figure, not agency_price.
-- And the workshop's audience is the company's own people: a user who belongs to an end customer
-- (by role, or by being attached to one) is never counted as workshop, however their user row is
-- filed, so the wholesale price cannot reach them through the workshop view.
...
AS $function$
  SELECT DISTINCT ud.user_id
   ...
   WHERE q.quotation_id = p_quotation_id
     ...
     -- An end customer's person is the other audience, whatever company their row points at.
     AND ud.user_role IS DISTINCT FROM qvm_new_apps.customer_role_id(false)
     AND ud.user_role IS DISTINCT FROM qvm_new_apps.customer_role_id(true)
     AND NOT EXISTS (SELECT 1 FROM qvm_new_apps.end_customer_users eu WHERE eu.user_id = ud.user_id)
...
AS $function$
  SELECT COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
               ...
               -- تسعيرك column — سعر الجملة (price_before_vat) for the workshop, سعر العميل
               -- (agency_price) for the customer — falling back to the offer it would be filled from,
               -- exactly as the senders fill it.
               'unit_price', CASE WHEN p_audience = 'workshop'
                                  THEN COALESCE(li.wholesale_price, NULLIF(qi.price_before_vat, 0), bch.unit_price, bq.cost)
                                  ELSE COALESCE(li.customer_price, NULLIF(qi.agency_price, 0), cp.agency_price, bq.agency_price) END,
               'customer_price_reference',
                 CASE WHEN p_audience = 'workshop'
                      THEN COALESCE(li.customer_price, NULLIF(qi.agency_price, 0), cp.agency_price, bq.agency_price) END,
...
CREATE OR REPLACE FUNCTION public.approval_flow_version()
 RETURNS integer LANGUAGE sql STABLE AS $$ SELECT 4 $$;
