-- Synced from QVM/test branch applied migration history (version 20260407085203, name: qpd562_fix_attendance_integer)
BEGIN;

SET search_path TO qvm_new_apps, public;

-- 1) Availability: use list_data join for record_type (integer)
...
-- 2) Attendance RPC: accept record_type string or id, validate, prevent overlaps/duplicates
...
BEGIN
  ...
  v_type_id := COALESCE(NULLIF(p_payload->>'record_type_id','')::int, (
    SELECT ld.list_data_id FROM qvm_new_apps.list_data ld WHERE lower(ld.list_data) = v_type_str LIMIT 1
  ));
  SELECT lower(ld.list_data) INTO v_type_name FROM qvm_new_apps.list_data ld WHERE ld.list_data_id = v_type_id;
    ...
    IF EXISTS (
      ...
      WHERE a.account_manager = v_manager
        AND a.record_type IN (
          SELECT ld.list_data_id FROM qvm_new_apps.list_data ld WHERE lower(ld.list_data) = 'vacation'
        )
  ...
  -- prevent duplicates for excuse/overtime (same date/slot/type)
  IF v_type_name IN ('excuse','overtime') THEN
    IF EXISTS (
      SELECT 1 FROM qvm_new_apps.account_manager_attendance a
      WHERE a.account_manager = v_manager
...
-- 3) Safe cron schedule via wrapper (ignore if not permitted)
