-- Synced from QVM/test branch applied migration history (version 20260325125535, name: qpd385_fix_filtered_cte_scope)
-- QPD-385: Fix CTE scope error in get_purchase_invoices_dashboard (relation "filtered" does not exist)
...
BEGIN
  ...
  -- Second statement: compute rows (duplicate CTEs since CTE scope is per-statement)
  WITH delivered AS (
    SELECT sl.confirmed_item_id,
           sl.status_changed_by,
           sl.created_at,
