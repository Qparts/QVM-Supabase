-- Synced from QVM/test branch applied migration history (version 20260407094809, name: qpd445_manage_slots)
BEGIN;

SET search_path TO qvm_new_apps, public;

CREATE TABLE IF NOT EXISTS qvm_new_apps.weekly_daysoff (
...
BEGIN
  ...
  FROM (
    ...
    LEFT JOIN qvm_new_apps.weekly_daysoff w ON w.account_manager = m.user_id AND w.month = v_m
  ) x;
...
BEGIN
  ...
  IF p_changes ? 'day_off' THEN
    ...
    IF v_day_off IS NOT NULL AND v_day_off BETWEEN 0 AND 6 THEN
      INSERT INTO qvm_new_apps.weekly_daysoff(account_manager, month, day_off)
...
BEGIN
  ...
  FOR rec IN SELECT * FROM jsonb_array_elements(p_rows) LOOP
    ...
    IF p_update_weekly_dayoff THEN
      v_day_off := NULLIF(rec->>'day_off','')::smallint;
      IF v_day_off IS NOT NULL AND v_day_off BETWEEN 0 AND 6 THEN
        INSERT INTO qvm_new_apps.weekly_daysoff(account_manager, month, day_off) VALUES (v_manager, v_m, v_day_off)
        ON CONFLICT (account_manager, month) DO UPDATE SET day_off = EXCLUDED.day_off;
        INSERT INTO qvm_new_apps.weekly_daysoff(account_manager, month, day_off) VALUES (v_manager, v_m_next, v_day_off)
        ON CONFLICT (account_manager, month) DO UPDATE SET day_off = EXCLUDED.day_off;
      END IF;
    END IF;
  END LOOP;
