-- Synced from QVM/test branch applied migration history (version 20260705115356, name: trigger_write_price_to_sheet_on_priced)
-- Trigger write_price_to_sheet edge function when quotation_items.item_status changes to Priced (17)
BEGIN;
...
DECLARE
  ...
  v_headers jsonb;
BEGIN
  IF NEW.item_status IS DISTINCT FROM OLD.item_status AND NEW.item_status = 17 THEN
