-- Rollback of 20260708130000: show every vendor user again (including Vendor Admins), not just
-- branch-restricted ones. The row label no longer names the vendor company, so an admin-vendor
-- row doesn't reveal which company it belongs to either — filtering them out is no longer needed.
