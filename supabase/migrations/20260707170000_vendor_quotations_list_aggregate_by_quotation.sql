-- Fix: get_vendor_quotations returned one row per quotation_vendors row (i.e. per branch), so a
-- single RFQ routed to several of the vendor's branches showed up as several near-duplicate list
-- rows, each counting only its own branch's parts (0/0, 3/3, ...). Meanwhile the detail page
-- aggregates across branches, so the list numbers never matched the detail and pagination counted
-- branch rows instead of quotations.
--
-- Now aggregate per quotation_id across the vendor's matching branches: one row per RFQ, with parts
-- de-duplicated by part, a summed total, MIN vendor_status, and a priced_parts count so the frontend
-- no longer needs to enrich it per-branch. Pagination/total_count now count distinct quotations.
