-- The workshop's request for a better price goes back to the vendors who gave it.
--
-- «طلب تحسين السعر» reached the pricing page as a banner and stopped there: a vendor line, once
-- priced, is locked on the vendor's own grid, so the buyer had no way to put the request in front
-- of the people who set the number. Now the buyer re-sends the affected lines to the same vendors
-- with the workshop's note. On the vendor's side the line reopens — their previous price kept in
-- view, the note beside it — and the next price they save closes the request and flows back
-- through the same path as any other price.
--
-- The line is reopened by putting vendor_item_status back to 157 (sent, unanswered). That is the
-- status every vendor screen already treats as editable, and update_vendor_status rolls it up to
-- the vendor's order status, so the vendor's dashboard shows the order as needing them again.

ALTER TABLE qvm_new_apps.quotation_vendor_items
  ADD COLUMN IF NOT EXISTS improvement_requested_at timestamptz,
  ADD COLUMN IF NOT EXISTS improvement_note text,
  ADD COLUMN IF NOT EXISTS previous_cost numeric(12,2);

-- Re-send the chosen lines to every vendor who priced them. Returns the vendors touched, with
-- their quote links, so the page can tell them; the mail goes from the page like every other one.
CREATE OR REPLACE FUNCTION qvm_new_apps.request_price_improvement(
  p_quotation_id bigint, p_note text, p_item_ids bigint[] DEFAULT NULL)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  IF NOT qvm_new_apps.is_qparts_team() THEN
    RAISE EXCEPTION 'Only the Qparts team can ask vendors for a better price';
  END IF;

  WITH reopened AS (
    UPDATE qvm_new_apps.quotation_vendor_items qvi
       SET previous_cost            = COALESCE(NULLIF(qvi.cost, 0), qvi.previous_cost),
           improvement_requested_at = now(),
           improvement_note         = NULLIF(btrim(COALESCE(p_note, '')), ''),
           vendor_item_status       = 157,
           best_cost                = false,
           updated_at               = now()
     ...
     WHERE qi.quotation_item_id = qvi.quotation_item_id
       ...
       -- Only lines a vendor actually priced: an unanswered line is already open, and a vendor who
       -- said "not available" (161) has nothing to improve.
       AND qvi.cost IS NOT NULL AND qvi.cost > 0
       AND COALESCE(qvi.vendor_item_status, 0) IN (158)
    RETURNING qvi.quotation_vendor_id
  )
  SELECT count(*) INTO v_lines FROM reopened;

  -- Roll the reopened lines up to each vendor's order status, and give their mailed link a fresh
  -- week: the request is a new ask, and a link that expired last month should not be the reason
  -- they cannot answer it.
  FOR v_qv IN
    SELECT DISTINCT qvi.quotation_vendor_id
     ...
     WHERE qi.quotation_id = p_quotation_id AND qvi.improvement_requested_at IS NOT NULL
  LOOP
    PERFORM qvm_new_apps.update_vendor_status(v_qv);
    UPDATE qvm_new_apps.quotation_vendors
       SET token_expires_at = GREATEST(token_expires_at, now() + interval '7 days')
     WHERE quotation_vendor_id = v_qv;
  END LOOP;
  ...
  RETURN jsonb_build_object(
    ...
    'vendors', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
               ...
               'vendor_id',        qv.vendor_id,
               'vendor_branch_id', qv.vendor_branch_id,
               'vendor_name',      v.vendor_name,
               'access_token',     qv.access_token,
               'lines', (SELECT count(*) FROM qvm_new_apps.quotation_vendor_items x
                          WHERE x.quotation_vendor_id = qv.quotation_vendor_id
                            AND x.improvement_requested_at IS NOT NULL)))
       ...
       WHERE qv.quotation_id = p_quotation_id
         AND EXISTS (SELECT 1 FROM qvm_new_apps.quotation_vendor_items x
                      WHERE x.quotation_vendor_id = qv.quotation_vendor_id
                        AND x.improvement_requested_at IS NOT NULL)), '[]'::jsonb));
END;
$function$;

CREATE OR REPLACE FUNCTION public.request_price_improvement(p_quotation_id bigint, p_note text, p_item_ids bigint[] DEFAULT NULL)
 RETURNS jsonb LANGUAGE sql SECURITY DEFINER SET search_path TO 'qvm_new_apps', 'public'
AS $$ SELECT qvm_new_apps.request_price_improvement(p_quotation_id, p_note, p_item_ids); $$;
REVOKE ALL ON FUNCTION public.request_price_improvement(bigint, text, bigint[]) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.request_price_improvement(bigint, text, bigint[]) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION qvm_new_apps.request_price_improvement(bigint, text, bigint[]) TO authenticated, service_role;

-- ── A new price closes the request ────────────────────────────────────────────────────────────
...
-- ── The vendor's grid sees the request ───────────────────────────────────────────────────────
