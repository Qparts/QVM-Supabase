-- The marketplace, as one read.
--
-- Every card on the page in one call: what is on offer, what this company is on, what it has left,
-- and — for the connect-type services — whether its own account is attached. Seven cards each
-- fetching their own state would be seven round trips and seven chances for the page to show a
-- subscription that the balance no longer supports.
--
-- `can_subscribe` is answered here rather than derived on screen. Whether a service is available,
-- whether it is already taken, whether the wallet covers it: all three are server facts, and a
-- button enabled by a browser's opinion of them is a button that fails on click.
--
-- NOTE: the usage join reads `u.scope is null` when first applied; migration 20260921194532
-- rewrites it to `u.scope = ''` along with the column it depends on.
...
revoke all on function qvm_new_apps.integrations_market(integer) from public;
grant execute on function qvm_new_apps.integrations_market(integer) to authenticated, service_role;
