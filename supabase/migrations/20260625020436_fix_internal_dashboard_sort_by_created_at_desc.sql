-- Synced from QVM/test branch applied migration history (version 20260625020436, name: fix_internal_dashboard_sort_by_created_at_desc)
...
CREATE OR REPLACE FUNCTION public.get_internal_dashboard(
  ...
  p_account_managers text[] DEFAULT NULL,
  p_clients integer[] DEFAULT NULL,
  p_branches integer[] DEFAULT NULL,
  p_brands integer[] DEFAULT NULL,
...
BEGIN
  ...
  paged_filtered AS (
    ...
    ORDER BY rfq_date DESC, quotation_id DESC
    ...
    'data', COALESCE(
      (
        SELECT jsonb_agg(
          jsonb_build_object(
            ...
            'items', (
              SELECT jsonb_agg(
                ...
                ORDER BY qi.created_at DESC, qi.quotation_item_id DESC
              )
          ...
          ORDER BY fwb.rfq_date DESC, fwb.quotation_id DESC
        )
