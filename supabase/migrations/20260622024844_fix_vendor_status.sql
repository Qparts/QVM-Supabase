-- Synced from QVM/test branch applied migration history (version 20260622024844, name: fix_vendor_status)
...
AS $function$DECLARE
  ...
  updated_count int := 0;
BEGIN
  -- Compute aggregate vendor_status from quotation_vendor_items
  WITH items AS (
  ...
  SELECT CASE
           ...
           WHEN EXISTS (SELECT 1 FROM items WHERE vendor_item_status = 158) THEN 158
  ...
  -- Always update when we have a valid computed status
