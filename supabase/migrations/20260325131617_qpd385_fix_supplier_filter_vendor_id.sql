-- Synced from QVM/test branch applied migration history (version 20260325131617, name: qpd385_fix_supplier_filter_vendor_id)
-- QPD-385: Correct supplier filter in get_purchase_invoices_dashboard to use vendor_id
SET search_path TO qvm_new_apps, public;

-- Optional supporting index (not harmful if redundant)
CREATE INDEX IF NOT EXISTS idx_qvi_vendor_id ON quotation_vendor_items(vendor_id);
