-- Synced from QVM/test branch applied migration history (version 20260621045836, name: fix_change_item_status_bulk_populate_delivery_notes)
...
begin
  ...
  if lower(trim(v_new_status_label)) = 'delivered' then
    ...
    resolved_deliveries as (
      ...
      from target_orders to2
      left join inserted_deliveries id
        on id.confirmed_order_id = to2.confirmed_order_id
      left join lateral (
    ...
    -- Also populate delivery_notes so the Delivered Orders page can display them
    ...
    resolved_returns as (
      ...
      from target_orders to2
      left join inserted_returns ir
        on ir.confirmed_order_id = to2.confirmed_order_id
      left join lateral (
