-- Synced from QVM/test branch applied migration history (version 20260325131738, name: qpd385_optimize_missing_rn_branch)
-- QPD-385: Optimize get_purchase_invoices_dashboard for missing RN filter using conditional plan
...
BEGIN
  IF p_missing_rn THEN
    -- Fast path for Missing Return Invoices: avoid joining credit notes, use NOT EXISTS
  ...
  ELSE
    -- General path (includes credit notes join to show URL)
    WITH delivered_ids AS (
      SELECT array_agg(list_data_id) AS ids
      FROM list_data
      WHERE lower(list_data) LIKE 'deliver%'
