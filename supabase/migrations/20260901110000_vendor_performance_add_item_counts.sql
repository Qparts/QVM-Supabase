-- Vendor Performance report: each existing metric currently counts distinct vendor-quotation
-- requests (or purchase orders, for "confirmed"). Adds the underlying item count beside each, so
-- the report shows both "how many requests/orders" and "how many parts" per bucket.
