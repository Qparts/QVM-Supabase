-- Synced from QVM/test branch applied migration history (version 20260405073500, name: qpd423_isolated_po_per_upload)
-- QPD-423: Isolate each upload into a new purchase_order when mode='replace'
BEGIN;
...
BEGIN
  ...
  IF COALESCE(p_mode,'add') = 'replace' THEN
    -- Create a fresh purchase_order to isolate this upload
  ...
  ELSE
    -- Reuse latest or create if none exists
