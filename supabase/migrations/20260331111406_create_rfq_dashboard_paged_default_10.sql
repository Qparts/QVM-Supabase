-- Synced from QVM/test branch applied migration history (version 20260331111406, name: create_rfq_dashboard_paged_default_10)
...
BEGIN
  ...
  FROM (
    ...
    -- First item per quotation for vehicle info
    LEFT JOIN LATERAL (
      SELECT
        qi3.vin AS vin,
    ...
    -- Status info and permission-filtered items aggregation
    LEFT JOIN LATERAL (
      SELECT 
