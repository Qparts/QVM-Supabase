-- QPD-304: Internal Dashboard RPC Function
-- Comprehensive view of all RFQs and Orders for internal users with advanced filtering
...
CREATE OR REPLACE FUNCTION public.get_internal_dashboard(
  ...
  p_mode text DEFAULT 'regular', -- 'regular' or 'bulk'
...
DECLARE
  ...
  v_result jsonb;
BEGIN
  -- Get user type to ensure only internal users can access
  SELECT user_type
  INTO v_user_type
  ...
  WITH filtered_quotations AS (
    SELECT DISTINCT
      ...
      -- Get first item's customer for branch lookup
      (
    ...
    WHERE
      ...
      -- Search filter (order number, plate)
      AND (
        p_search IS NULL
        ...
        SELECT jsonb_agg(
          jsonb_build_object(
            ...
            'account_manager_history', (
              SELECT jsonb_agg(
                jsonb_build_object(
                  'account_manager_id', qam.account_manager_id,
                  'account_manager_name', am_hist.user_name,
                  'assigned_at', qam.assigned_at
                )
                ORDER BY qam.assigned_at DESC
              )
              FROM qvm_new_apps.quotation_account_managers qam
              LEFT JOIN qvm_new_apps.user_data am_hist ON am_hist.user_id = qam.account_manager_id
            ...
            'items', (
              ...
              WHERE qi.quotation_id = fwb.quotation_id
                ...
                -- Search filter (part number, description, VIN, final part number)
                AND (
                  p_search IS NULL
...
-- Helper function to get account managers list for filter
CREATE OR REPLACE FUNCTION public.get_account_managers_list(
  p_user_id uuid
)
RETURNS jsonb
LANGUAGE plpgsql
...
BEGIN
  ...
  RETURN (
    ...
    WHERE ud.user_type = 185 -- Internal users only
      AND ud.user_id IS NOT NULL
  );
END;
$function$;

REVOKE EXECUTE ON FUNCTION public.get_account_managers_list(uuid) FROM public, anon;
GRANT EXECUTE ON FUNCTION public.get_account_managers_list(uuid) TO authenticated;

COMMENT ON FUNCTION public.get_internal_dashboard IS 'QPD-304: Fetch comprehensive internal dashboard data with advanced filtering for RFQs and Orders';
COMMENT ON FUNCTION public.get_account_managers_list IS 'QPD-304: Get list of account managers for filter dropdown';
