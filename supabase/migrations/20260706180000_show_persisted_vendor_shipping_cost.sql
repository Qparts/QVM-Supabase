-- Show the already-persisted per-item vendor shipping cost (set at PO-creation time) when the
-- Pricing page is reopened for an order that already has a purchase order raised for an item.
