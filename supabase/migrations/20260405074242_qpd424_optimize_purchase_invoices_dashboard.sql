-- Synced from QVM/test branch applied migration history (version 20260405074242, name: qpd424_optimize_purchase_invoices_dashboard)
-- QPD-424: Optimize get_purchase_invoices_dashboard performance
BEGIN;

SET search_path TO qvm_new_apps, public;

-- Performance indexes
CREATE INDEX IF NOT EXISTS idx_purchase_items_confirmed_item ON qvm_new_apps.purchase_items(confirmed_item_id, purchase_order_id);
CREATE INDEX IF NOT EXISTS idx_purchase_orders_coid_created_at ON qvm_new_apps.purchase_orders(confirmed_order_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_status_logs_item_delivered ON qvm_new_apps.status_logs(confirmed_item_id, item_status, created_at DESC);

-- Optimized dashboard RPC: single pass for counts + rows, precompute latest PO per item
...
BEGIN
  ...
  user_ctx AS (
    ...
    SELECT confirmed_item_id, purchase_order_id, vendor_invoice_url, vendor_invoice_number, zoho_bill_url
    FROM (
