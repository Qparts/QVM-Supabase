...
-- Two consequences of email conversations having no WhatsApp number.
--
-- 1. The list filtered every row on `wa_account_id = any(my numbers)`, and an email
--    thread's is null, so nothing that arrived by mail was ever shown. With a number
--    picked the filter still means what it says — that number's conversations — so mail
--    appears in the unified view, which is the view that is about all of them.
--
-- 2. Every outgoing mail was subjected «Re: …», including the first one. Re: is an answer;
--    on a conversation we started it makes us look like we lost the thread. It is now
--    added only when the vendor has actually written to us here.
  ...
  with scoped as (
    select t.thread_id, t.status, t.unread_count, t.last_message_at, t.last_message_preview,
     ...
     where t.deleted_at is null
       and (case
              -- A mailbox belongs to no number, so it belongs to the view that is about
              -- every conversation rather than to one number's.
              when p_wa_account_id is null then t.channel = 'email' or t.wa_account_id = any(v_ids)
              else t.wa_account_id = any(v_ids)
            end)
  ),
