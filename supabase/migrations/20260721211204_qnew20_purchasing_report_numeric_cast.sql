...
begin
  ...
  select
    ...
    qvi.vendor_id,
    coalesce(pi.final_purchase_price, qvi.cost)::numeric AS chosen_cost,
    coalesce(pi.vendor_shipping_cost, qvi.item_shipping, 0)::numeric AS chosen_shipping,
  ...
  select jsonb_build_object(
    ...
    'samples', coalesce(jsonb_agg(jsonb_build_object(
        'part_number', pp.part_number, 'chosen', round(pp.chosen_cost,2),
  ...
  select jsonb_build_object(
    ...
    'samples', coalesce(jsonb_agg(jsonb_build_object(
        'part_number', part_number, 'chosen', round(chosen_cost,2),
