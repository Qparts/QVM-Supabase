-- The companies pane is ordered by age, then by size.
--
-- It was alphabetical, which answers a question nobody was asking: the company someone added five
-- minutes ago sat wherever its name happened to fall, and finding it meant reading the list.
--
-- Newest first puts it at the top. That leaves one large tie — every company carried over from
-- list 1 was stamped with the same created_at by the backfill that made them — and the number of
-- workshops breaks it, so within that block the substantial companies come first and the empty
-- shells sink. The name is the last resort, for the pair that match on both.
  ...
  SELECT jsonb_build_object(
    ...
    'data', jsonb_build_object(
      ...
      'unassigned_workshops', CASE WHEN qvm_new_apps.is_qparts_admin(v_uid) THEN
        ...
        ELSE '[]'::jsonb END,
      'companies', COALESCE((
        -- Newest first, and the many companies that share a created_at — everything carried over
        -- from list 1 was stamped in the same instant by the backfill — broken by how many
        -- workshops each one has. Alphabetical put "Al…" at the top and the company someone added
        -- five minutes ago wherever its name happened to fall.
        SELECT jsonb_agg(co ORDER BY s.created_at DESC NULLS LAST, s.workshop_count DESC, co->>'display_name')
        FROM (
          SELECT jsonb_build_object(
