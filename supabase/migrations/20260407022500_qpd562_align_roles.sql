...
-- list_account_managers: align roles; prefer 'qparts account manager' but accept 'account manager'
...
BEGIN
  SELECT EXISTS (
    ...
    WHERE u.user_id = p_user_id
      AND (
        u.user_type = 185
        OR lower(ur.list_data) IN ('qparts admin','admin','pricing supervisor')
      )
  ) INTO v_allowed;
  ...
  FROM (
    SELECT u.user_id AS id, COALESCE(u.user_name, u.email)::text AS name
    FROM qvm_new_apps.user_data u
...
-- get_account_manager_branches_dashboard: align editor role to qparts admin or internal (keep pricing supervisor for compatibility)
...
-- replace_account_manager_branches_by_names: accept either role label when resolving names
...
BEGIN
  ...
  FOR rec IN SELECT * FROM jsonb_array_elements(p_rows) LOOP
    ...
    IF m1 IS NOT NULL THEN SELECT u.user_id INTO um1 FROM qvm_new_apps.user_data u LEFT JOIN qvm_new_apps.list_data r ON r.list_data_id = u.user_role WHERE lower(u.user_name) = lower(m1) AND lower(r.list_data) IN ('qparts account manager','account manager') LIMIT 1; END IF;
    IF m2 IS NOT NULL THEN SELECT u.user_id INTO um2 FROM qvm_new_apps.user_data u LEFT JOIN qvm_new_apps.list_data r ON r.list_data_id = u.user_role WHERE lower(u.user_name) = lower(m2) AND lower(r.list_data) IN ('qparts account manager','account manager') LIMIT 1; END IF;
    IF m3 IS NOT NULL THEN SELECT u.user_id INTO um3 FROM qvm_new_apps.user_data u LEFT JOIN qvm_new_apps.list_data r ON r.list_data_id = u.user_role WHERE lower(u.user_name) = lower(m3) AND lower(r.list_data) IN ('qparts account manager','account manager') LIMIT 1; END IF;
    IF s11 IS NOT NULL THEN SELECT u.user_id INTO us11 FROM qvm_new_apps.user_data u LEFT JOIN qvm_new_apps.list_data r ON r.list_data_id = u.user_role WHERE lower(u.user_name) = lower(s11) AND lower(r.list_data) IN ('qparts account manager','account manager') LIMIT 1; END IF;
    IF s12 IS NOT NULL THEN SELECT u.user_id INTO us12 FROM qvm_new_apps.user_data u LEFT JOIN qvm_new_apps.list_data r ON r.list_data_id = u.user_role WHERE lower(u.user_name) = lower(s12) AND lower(r.list_data) IN ('qparts account manager','account manager') LIMIT 1; END IF;
    IF s13 IS NOT NULL THEN SELECT u.user_id INTO us13 FROM qvm_new_apps.user_data u LEFT JOIN qvm_new_apps.list_data r ON r.list_data_id = u.user_role WHERE lower(u.user_name) = lower(s13) AND lower(r.list_data) IN ('qparts account manager','account manager') LIMIT 1; END IF;
    IF s21 IS NOT NULL THEN SELECT u.user_id INTO us21 FROM qvm_new_apps.user_data u LEFT JOIN qvm_new_apps.list_data r ON r.list_data_id = u.user_role WHERE lower(u.user_name) = lower(s21) AND lower(r.list_data) IN ('qparts account manager','account manager') LIMIT 1; END IF;
    IF s22 IS NOT NULL THEN SELECT u.user_id INTO us22 FROM qvm_new_apps.user_data u LEFT JOIN qvm_new_apps.list_data r ON r.list_data_id = u.user_role WHERE lower(u.user_name) = lower(s22) AND lower(r.list_data) IN ('qparts account manager','account manager') LIMIT 1; END IF;
    IF s23 IS NOT NULL THEN SELECT u.user_id INTO us23 FROM qvm_new_apps.user_data u LEFT JOIN qvm_new_apps.list_data r ON r.list_data_id = u.user_role WHERE lower(u.user_name) = lower(s23) AND lower(r.list_data) IN ('qparts account manager','account manager') LIMIT 1; END IF;
    IF fb  IS NOT NULL THEN SELECT u.user_id INTO ufb  FROM qvm_new_apps.user_data u LEFT JOIN qvm_new_apps.list_data r ON r.list_data_id = u.user_role WHERE lower(u.user_name) = lower(fb)  AND lower(r.list_data) IN ('qparts account manager','account manager') LIMIT 1; END IF;
  ...
  FOR rec IN SELECT * FROM jsonb_array_elements(p_rows) LOOP
    ...
    INSERT INTO qvm_new_apps.account_manager_branches(
      ...
      (b_id, 1, um1, us11, us21, ufb, now(), now()),
      (b_id, 2, um2, us12, us22, ufb, now(), now()),
      (b_id, 3, um3, us13, us23, ufb, now(), now());
