-- QVM/test currently has 4 quotations with zero rows in quotation_items (a data-entry gap - an
-- RFQ record created with no line items ever added). get_internal_dashboard's p_view='rfqs'
-- branch includes any quotation with no confirmed_order regardless of whether it has any items
-- at all, so these empty quotations were showing up in the Internal Dashboard's RFQ list.
-- Excluding quotations with no items at all (independent of any brand/status/search filter,
-- which already narrow the 'items' array separately) fixes this without changing any other
-- filtering behavior.
