-- Purchase Invoices table: show who created the purchase order.
--
-- purchase_orders.created_by is empty on 337 of 338 rows on this branch. The trg_set_created_by
-- trigger works, but the "Send PO" path gave it no user to find: send_po_webhook holds its
-- transaction on a raw Postgres connection, which carries no JWT, so auth.uid() was NULL. That is
-- fixed in the edge function (see the send_po_webhook change on this branch); this exposes the
-- resulting value so the dashboard can show it.
--
-- Falls back to uploaded_by, which is set by the upsert_purchase_order_items path and is the only
-- attribution the historical rows have. Rows with neither stay null and render as a dash — the
-- webhook logs record no user, so there is nothing to backfill them from.
--
-- Scoped deliberately to get_purchase_invoices_dashboard, the item-level table master actually
-- renders. The PO-level receiving dashboard is dev-only and does not exist here.

CREATE OR REPLACE FUNCTION public.get_purchase_invoices_dashboard(p_user_id uuid, p_is_manager boolean DEFAULT false, p_search text DEFAULT NULL::text, p_missing_pi boolean DEFAULT false, p_missing_rn boolean DEFAULT false, p_account_manager uuid DEFAULT NULL::uuid, p_branch_ids integer[] DEFAULT NULL::integer[], p_supplier_ids integer[] DEFAULT NULL::integer[], p_limit integer DEFAULT 200, p_offset integer DEFAULT 0)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  latest_po_by_item AS (
    SELECT DISTINCT ON (pi.confirmed_item_id)
      ...
      -- Who raised the PO. Falls back to uploaded_by for orders created before
      -- send_po_webhook replayed its caller's identity onto the connection.
      COALESCE(po.created_by, po.uploaded_by) AS po_created_by,
    ...
    'rows', COALESCE((
      SELECT jsonb_agg(to_jsonb(t)) FROM (
        SELECT
          ...
          (SELECT user_name FROM qvm_new_apps.user_data WHERE user_id = po_created_by) AS po_created_by_name,
          po_created_at,
