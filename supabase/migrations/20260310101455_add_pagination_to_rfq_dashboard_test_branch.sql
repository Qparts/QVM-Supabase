-- Synced from QVM/test branch applied migration history (version 20260310101455, name: add_pagination_to_rfq_dashboard_test_branch)
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'total_count', (SELECT COUNT(*) FROM base_rows),
    'data', COALESCE(
      jsonb_agg(rq ORDER BY (rq->>'created_at')::timestamptz DESC),
      '[]'::jsonb
    )
  )
  INTO v_result
  FROM (
    SELECT rq
    FROM base_rows
    ORDER BY (rq->>'created_at')::timestamptz DESC
...
REVOKE EXECUTE ON FUNCTION public.rfq_dashboard(
  uuid, text, text, text, text, uuid, timestamptz, timestamptz, integer, integer
) FROM public, anon;

GRANT EXECUTE ON FUNCTION public.rfq_dashboard(
  uuid, text, text, text, text, uuid, timestamptz, timestamptz, integer, integer
) TO authenticated;
;
