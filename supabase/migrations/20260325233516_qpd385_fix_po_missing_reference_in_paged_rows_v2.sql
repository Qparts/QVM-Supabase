-- Synced from QVM/test branch applied migration history (version 20260325233516, name: qpd385_fix_po_missing_reference_in_paged_rows_v2)
-- QPD-385: Fix CTE scoping and syntax; ensure po_missing/rn_missing exist in all statements
...
DECLARE
  ...
  total_orders int;
BEGIN
  -- Cache delivered status ids once
  ...
  WITH user_ctx AS (
    ...
    SELECT ci.confirmed_item_id, co.confirmed_order_id, q.order_number, q.created_at AS rfq_date, co.created_at AS confirmation_date,
           q.account_manager, qi.customer_id, cb.branch_name, qi.model, ldb.list_data AS main_brand, qi.part_description,
           ci.final_part_number, ldf.list_data AS final_brand_class, ci.approved_qty, qvi.cost AS purchase_cost, qvi.vendor_id,
           ldv.list_data AS vendor_name, po.vendor_invoice_url, po.vendor_invoice_number, po.zoho_bill_url,
  ...
  FROM (
    SELECT confirmed_item_id, confirmed_order_id, order_number, rfq_date, confirmation_date,
           (SELECT user_name FROM qvm_new_apps.user_data WHERE user_id = delivered_by) AS delivered_by_name,
           vendor_invoice_url, vendor_invoice_number, zoho_bill_url, branch_name, model, main_brand,
           part_description, final_part_number, final_brand_class, approved_qty, purchase_cost, vendor_name
    FROM filtered
    ORDER BY coalesce(confirmation_date, rfq_date) DESC, order_number, confirmed_item_id
    LIMIT p_limit OFFSET p_offset
  ) t;

  RETURN jsonb_build_object('status','success','message','OK','total', total_rows,'orders_total', total_orders,'rows', rows);
END;
$function$;
