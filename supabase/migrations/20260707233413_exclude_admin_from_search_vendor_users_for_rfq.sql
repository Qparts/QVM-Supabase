-- Synced from QVM/test branch applied migration history (version 20260707233413, name: exclude_admin_from_search_vendor_users_for_rfq)
-- Only branch-restricted "Vendor" users are useful to search/select directly here — a Vendor Admin
-- isn't scoped to specific branches (selecting the vendor company row already gives full access),
-- so listing them alongside branch-restricted logins just adds noise without narrowing anything.
...
BEGIN
  ...
  WHERE u.user_type = 205
    AND coalesce(ur.list_data, '') <> 'Vendor Admin'
    AND (
