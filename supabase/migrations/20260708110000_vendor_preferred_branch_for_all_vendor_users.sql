-- Let ANY vendor user (not just the admin-vendor) set the company's preferred branch — a
-- branch-restricted "Vendor" user may only choose a branch they're personally assigned to
-- (or clear the preference), while the admin-vendor keeps full control over any branch.
...
-- Surface the vendor's current preferred_branch_id in the JWT-backing user_data payload, so the
-- navbar branch switcher (available to every vendor user, admin or not) can show/toggle it without
-- an extra round trip.
