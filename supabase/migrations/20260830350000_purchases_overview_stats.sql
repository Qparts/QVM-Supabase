-- New "Purchases Performance Reports" tab: first block — 4 system-wide KPI numbers.
--
--   purchase_invoice_count = count of qvm_new_apps.purchase_invoice_attachments rows (the actual
--     vendor-invoice-upload records — verified live that purchase_orders.vendor_invoice_url is
--     never populated, so this attachments table is the real source, not that column).
--   purchase_order_count = count of qvm_new_apps.purchase_orders rows.
--   avg_delivery_hours = avg time from "Out for Delivery" (qvm_new_apps.delivery_items.created_at,
--     the OOFD-signing record) to "Delivered" (qvm_new_apps.delivery_notes.created_at), system-wide.
--     Verified live: the only 2 delivery_items/delivery_notes pairs that exist today have IDENTICAL
--     timestamps (both signed in the same instant), so there is currently no strictly-positive
--     duration to average — this will legitimately return NULL until real data accumulates; not a
--     bug.
--   avg_purchasing_hours = avg time from the RFQ being sent to a vendor
--     (qvm_new_apps.quotation_vendors.created_at) to the item's selling price being set
--     (qvm_new_apps.pricing_logs.created_at, earliest per item), for the vendor that ultimately won
--     that item (qi.cost_id = qvi.cost_id) — same "Priced" signal established for the lifecycle
--     report (pricing_logs, not item_status, since item_status=17 is a manually-toggled, unreliable
--     signal per that report's own findings).
...
GRANT EXECUTE ON FUNCTION qvm_new_apps.get_purchases_overview_stats(integer, timestamptz, timestamptz) TO authenticated;
