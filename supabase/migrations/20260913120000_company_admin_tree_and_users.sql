-- The tree a Company Admin sees, and the people they may place in it.
--
-- admin_get_client_tree is the screen's whole content, so filtering it IS the permission: their own
-- companies, their workshops, their branches. The unassigned pile stays with Qparts, since a
-- workshop with no company belongs to nobody's.
--
-- admin_set_user_scope has to check each thing named rather than the caller alone, or a Company
-- Admin could move somebody else's account into their own company.
