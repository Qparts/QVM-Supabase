-- QNEW: vendors can suggest new items on a quotation; an admin accepts/rejects before distribution.
-- Accept -> "Sent To Vendor" (visible to workshop, pushed to AppSheet, sendable to other vendors).
-- Reject -> "Cancelled" (optional vendor notification). Applied to production via MCP; this file is
-- the repo copy of the final state.

-- 1) New item statuses (list_id = 3).
...
-- 3) Vendor adds a new item (logged-in vendor portal). Uses auth.uid() as created_by.
...
BEGIN
  ...
  INSERT INTO qvm_new_apps.quotation_vendor_items (
    quotation_item_id, vendor_id, quotation_vendor_id, best_cost, cost, from_database, vendor_item_status, created_at, updated_at
  ) VALUES (v_item_id, v_vendor, v_qv_id, false, p_cost, false, NULL, now(), now());
...
-- 4) Admin accept/reject a vendor-added item.
CREATE OR REPLACE FUNCTION public.review_vendor_added_item(
  p_quotation_item_id bigint, p_action text, p_reason text DEFAULT NULL
) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = ''
AS $$
DECLARE
  v_user uuid := auth.uid(); v_allowed boolean;
  v_cur int; v_added int; v_sent int; v_cancel int; v_new int; v_qid bigint; v_creator uuid; v_email text;
BEGIN
  IF v_user IS NULL THEN RETURN jsonb_build_object('status','error','message','Not authenticated'); END IF;
  SELECT EXISTS (
  ...
  UPDATE qvm_new_apps.quotation_items SET item_status = v_new, updated_at = now() WHERE quotation_item_id = p_quotation_item_id;

  IF v_new = v_cancel AND p_reason IS NOT NULL AND btrim(p_reason) <> '' THEN
    BEGIN
      PERFORM public.upsert_note_inline(p_note_type := 'quotation_items', p_type_id := p_quotation_item_id,
...
-- 5) AppSheet timing: skip vendor-suggested items on insert (pushed on admin accept instead).
CREATE OR REPLACE FUNCTION qvm_new_apps.trg_write_item_to_sheet_on_insert()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path = net, public AS $$
BEGIN
...
-- 6) Status-change sync skips transitions OUT of "Added by Vendor".
...
CREATE TRIGGER trg_push_vendor_item_on_accept
  ...
  FOR EACH ROW WHEN (NEW.item_status IS DISTINCT FROM OLD.item_status)
