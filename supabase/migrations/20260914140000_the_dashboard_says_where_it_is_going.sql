-- The internal dashboard says where the order is going.
--
-- Branches carry their own addresses now — several each, with a default — and the RFQ form asks
-- which one an order is for. The dashboard, which is where that order is worked on for the rest of
-- its life, showed none of it: whoever is arranging the delivery had to open another screen to find
-- out what they had agreed to.
--
-- An order that named an address shows that address. An order that named none — which is every
-- order raised before the field existed, and any raised on a branch with nothing on file — falls
-- back to the branch's default and says so, because "where this branch normally receives" is a
-- better answer than an empty column, as long as it does not pretend to be a decision somebody
-- made.
