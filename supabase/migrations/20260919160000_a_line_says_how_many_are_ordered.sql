-- A line says how many are ordered, and how many remain.
--
-- The Part Detail cell read "Requested 6" and nothing more. It now reads requested, ordered — the
-- quantity already on purchase orders for the line, cancelled purchase lines excluded — and the
-- remainder, so the desk sees at a glance what is still to buy.
...
CREATE OR REPLACE FUNCTION public.approval_flow_version()
 RETURNS integer LANGUAGE sql STABLE AS $$ SELECT 20 $$;
