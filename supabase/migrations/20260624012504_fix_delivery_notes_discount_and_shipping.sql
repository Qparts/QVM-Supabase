-- Synced from QVM/test branch applied migration history (version 20260624012504, name: fix_delivery_notes_discount_and_shipping)

-- Add shipping_price column to delivery_notes if not exists
ALTER TABLE qvm_new_apps.delivery_notes ADD COLUMN IF NOT EXISTS shipping_price numeric DEFAULT 0;
ALTER TABLE qvm_new_apps.return_notes ADD COLUMN IF NOT EXISTS shipping_price numeric DEFAULT 0;

-- Fix change_item_status_bulk: populate discount_percent, recalculate totals with qty and discount, add shipping_price
...
begin
  ...
  -- Block changing items that are already in the delivery/return workflow
  ...
  if lower(trim(v_new_status_label)) = 'delivered' then
    ...
    -- Populate delivery_notes with discount, correct totals, and shipping
    ...
    select
      ...
      -- total before vat = price * qty * (1 - discount/100)
